#ifndef CABINET_BIRTH_SURPRISE_PROBE_H
#define CABINET_BIRTH_SURPRISE_PROBE_H
 ...
 * by a believed cabinet is a candidate for a NEW cabinet: a concept condensing out of the residual field (birth =
 ...
 * surprise ramp on a real cabinet's region and confirm it flags births cleanly (and rejects phantoms) BEFORE
 * letting it gate/replace the tracker's frames-counter. Pure math (no DSR) → unit-testable. The removal half is
 * NOT here: the grid subtracts explained cells, so it cannot see a modelled cabinet's absence — removal stays the
 * per-instance existence-surprise job. See [[cabinet-birth-surprise-probe]].
 */
...
// A believed cabinet's footprint (room frame) — used to mark residual regions an existing concept already explains.
...
struct BirthCandidate
{
    ...
    bool  covered_by_cabinet = false;    // centroid inside a believed cabinet footprint (+margin) ⇒ NOT a birth
};

struct BirthProbeParams
{
    float p_threshold    = 0.5f;   // a cell counts as confidently-unexplained-occupied above this P
    float cover_margin_m = 0.30f;  // grow each cabinet footprint by this before the "already explained" test
    int   min_cells      = 4;      // ignore specks (sensor noise); a real cabinet footprint spans many cells
};
...
public:
    ...
    // MASS, flag whether a believed cabinet already covers it, and return them sorted by mass (strongest first).
    // Pure function of its inputs — no DSR, no state.
    static std::vector<BirthCandidate> scan(const GridField& g,
                                            const std::vector<FootprintBox>& cabinets,
                                            const BirthProbeParams& p = {})
    {
        ...
        for (int s = 0; s < N; ++s)
        {
            ...
            c.covered_by_cabinet = covered(c.cx, c.cy, cabinets, p.cover_margin_m);
            out.push_back(c);
        }
    ...
    // FUSION signal: a real cabinet detection should land on high residual mass; a flicker/phantom detection on ~0.
    ...
    // Self-test: two blobs (one already under a cabinet, one free); scan must find both, mark exactly one covered,
    ...
    static bool self_test()
    {
        ...
        std::vector<FootprintBox> cabinets{ {0.85f, 0.85f, 0.9f, 0.9f, 0.0f} };        // covers blob A
        const auto c = scan(g, cabinets, {});
        bool ok = c.size() == 2;                          // two real regions, speck dropped
        ok = ok and c[0].mass > c[1].mass;                // sorted by mass; B (bigger) first
        ok = ok and not c[0].covered_by_cabinet;            // blob B is free
        ok = ok and c[1].covered_by_cabinet;                // blob A sits under the cabinet
...
private:
    static bool covered(float x, float y, const std::vector<FootprintBox>& cabinets, float margin)
    {
        for (const auto& t : cabinets)
        {
            const float c = std::cos(t.yaw), s = std::sin(t.yaw);
            const float dx = x - t.cx, dy = y - t.cy;
            const float lx =  c * dx + s * dy;   // world → cabinet-local (rotate by −yaw)
...
#endif  // CABINET_BIRTH_SURPRISE_PROBE_H
