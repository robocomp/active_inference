/*
 * cabinet_affordance.h  —  lifecycle of the "affordance" DSR node for one cabinet.
 *
 * Manages an affordance node that hangs from a cabinet node via a "has_intention" edge. The node advertises
 * the robot pose that maximises epistemic value for this cabinet and tracks the mission-controller's progress
 ...
 * DSR edge type  : has_intention   (cabinet_node → affordance_node)
 *
...
// Owns the affordance node's create/refresh/remove + the controller-protocol state machine for one cabinet.
class CabinetAffordance
{
public:
    enum class State { idle, pending, executing, satisfied, aborted };

    CabinetAffordance() = default;

    /// Late initialisation — call once after the cabinet DSR node is known.
...
private:
    ...
    uint64_t    cabinet_node_id_   = 0;
    std::string cabinet_node_name_;
