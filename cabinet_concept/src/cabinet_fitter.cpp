 ...
 * + per-slice R inputs), and run_inference (lazy footprint-moment birth, support-bank ingest, one CabinetBelief
 ...
 * CabinetProjection, CabinetLidarRangeChannel, and the header-only support bank; SpecificWorker owns orchestration.
 */

#include "cabinet_fitter.h"
#include "../../common/diag_log/rotating_csv.h"   // keep the previous run instead of wiping it
#include "cabinet_support_bank.h"
...
void CabinetFitter::log_ai2_csv(const CabinetInstance& inst, int npts, float R, bool gated, float energy)
{
    ...
    if (not ai2_csv_.is_open())
    {
        ...
        ai2_csv_ << "cycle,node,pkt_fid,pkt_ts,npts,gated,energy,fe_baseline,fe_surprise,R,motion_var,depth_var,motion_dotd,ego_v,ego_w,ego_dt,trunc_frac,range,"
                 ...
                 << "axis_resid,cand_pts,resid_pts,nbv_gain_raw,nbv_gain_pub,ricoh_det,ricoh_attn\n";   // + Manhattan-yaw residual (rad) & candidate/residual split (merge diag)
    }
    ...
    ai2_csv_ << inst.processed_cycles << ',' << inst.node_name << ','
             ...
             << inst.ai2_belief.last_axis_resid() << ',' << inst.dbg_cand_pts << ',' << inst.dbg_resid_pts << ',' << inst.dbg_nbv_gain_raw << ',' << inst.dbg_nbv_gain_pub
