/*
 * cabinet_lidar_ingestor.h  —  lidar3D media-plane consumer for the YOLO-independent range factor
 *
 ...
 * LiDAR first-hit factor needs (see common/ai_belief/lidar_ray_factor.h). Because CabinetBelief exposes the
...
class CabinetLidarIngestor
{
public:
    CabinetLidarIngestor(std::shared_ptr<DSR::DSRGraph> graph, DSR::InnerEigenAPI* inner_eigen,
                       const CabinetConfig& cfg);
    ~CabinetLidarIngestor();
...
private:
    ...
    const CabinetConfig*                           cfg_ = nullptr;
