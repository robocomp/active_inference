...
void CabinetLidarRangeChannel::feed(CabinetInstance& inst, CabinetFrame& frame) const
{
    ...
    if (helios_on)
    {
        ...
        if (n > 0)
        {
            ...
            for (const auto& p : frame.lidar.endpoints) resid_sum += std::abs(inst.ai2_belief.sdf_box(p, s));
            inst.dbg_lidar_resid_m = static_cast<float>(resid_sum / n);
            // z-calibration probe: mean z of ALL selected returns + the highest/lowest 20% (≈ observed tabletop /
