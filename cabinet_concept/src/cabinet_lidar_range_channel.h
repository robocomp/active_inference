/*
 * cabinet_lidar_range_channel.h — YOLO-independent LiDAR range channel for cabinet_concept (extracted from CabinetFitter).
 *
 * Owns the staged per-cycle sweep (room frame + sensor origin) and selects the returns landing on ONE cabinet,
 * staging them on the frame's LiDAR channel for the first-hit range factor / free-space VACATE term:
 * set_sweep (stage this cycle's sweep + origin, from CabinetFitter fed by CabinetLidarIngestor), clear (drop the
 * staged sweep, called each cycle before set_sweep so a stale sweep never leaks), and feed (select this
 * cabinet's returns and populate frame.lidar). Holds the CabinetConfig by reference. Plain class (no Q_OBJECT).
 */
...
#include "cabinet_belief.h"     // rc::CabinetFrame

namespace rc {

class CabinetLidarRangeChannel
{
public:
    explicit CabinetLidarRangeChannel(const CabinetConfig& cfg) : cfg_(cfg) {}
    ...
    // Select this cycle's LiDAR returns landing on THIS cabinet and stage them on the frame's range channel(s).
    void feed(CabinetInstance& inst, CabinetFrame& frame) const;

private:
    // Select returns from `sweep` (sensor at `origin`) that land on this cabinet into `out`, with the two
    ...
    int select_into(const CabinetInstance& inst, const Eigen::Vector3f& centroid,
