      ...
      startup_check_flag(startup_check)
{
    ...
    // default silently.
    int period = 0;
    rc::cfg::Reader(configLoader, "cabinet_concept").req("Period.Compute", period,
...
void SpecificWorker::initialize()
{
    ...
    presence_protocol_.wire(
        presence_coordinator_, &statemachine, this, configLoader, G, static_cast<std::uint32_t>(agent_id),
        {
            ...
            .on_first_operating = [this] { remove_stale_affordance_nodes(); remove_owned_cabinet_nodes(); },
    ...
    // ARMS the unread sweep; check_registry_complete.sh cabinet_concept is the grep that keeps the claim
    // honest. Re-run it whenever a config read is added.
    rc::cfg::exempt_generated_prefixes();
    rc::cfg::registry().declare_complete("cabinet_concept");
    rc::cfg::Reader(configLoader, "cabinet_concept").publish("etc/config_effective.csv");
}
...
void SpecificWorker::log_birth_surprise()
{
    ...
    // Believed cabinet footprints (room frame) — a region under one is already explained, NOT a birth. This is
    // the one genuinely per-object step: a cabinet's two footprint extents. Everything after it — both CSVs,
    ...
    for (const auto& [id, inst] : fitter_->instances())
        ...
        { const auto& s = inst.ai2_belief.state(); footprints.push_back({s.cx, s.cy, s.L, s.d, s.yaw}); }

    birth_surprise_log_.write(residual_field_, footprints, last_cabinet_dets_xy_,
