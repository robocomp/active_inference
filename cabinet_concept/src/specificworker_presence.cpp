...
// keyed on cabinet's PRIMARY input (the retina `masks` node) instead of a LiDAR media plane.
...
void SpecificWorker::cleanup_owned_nodes()
{
    ...
    // and deleted each node by id — but [Owns].nodes already carries a WILDCARD ("cabinet*") that
