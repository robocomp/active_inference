 ...
 * chair_affordance.h
 *
 * Manages the lifecycle of an "affordance" DSR node that hangs from a chair
 * node via a "has_intention" edge.  The node advertises the robot pose that
 * maximises epistemic value for this chair, and tracks the mission-controller's
 ...
 * DSR edge type  : has_intention   (chair_node → affordance_node)
 *
...
namespace rc {
class ChairAffordance
{
public:
    enum class State { idle, pending, executing, satisfied, aborted };

    ChairAffordance() = default;

    /// Late initialisation — call once after the chair DSR node is known.
...
private:
    ...
    uint64_t    chair_node_id_   = 0;
    std::string chair_node_name_;
