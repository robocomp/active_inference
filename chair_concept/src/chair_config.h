...
struct ChairConfig
{
    ...
    // Schmitt hysteresis (anti-oscillation): a satisfied chair stays withdrawn for cooldown cycles AND
    ...
    float epistemic_rearm_info_gain = 5.0f;   // ΔH (nats) above which a satisfied chair re-arms
    ...
    // estimated chair-top height, dropping the floor / under-chair / clutter population that is
    ...
    // stays plastic — the stabiliser for successive viewpoints as the robot orbits the chair.
    ...
    // instead of the scalar-decay collapse toward zero. Physically Q = how much the chair pose/size
    ...
    // inset) groups so the chair CENTRE can be locked harder than its extents (position drifts one-sided
    ...
    // chair YOLO scores stay low (~0.3–0.45), so floor/ref must sit in that band or every mask is
    ...
    // Upload the chair pose covariance onto the room→chair RT edge (rt_covariance_att, 6×6 SE3),
    ...
    // OFF → legacy prior-scaffold + greedy nearest-mask. ON → masks are associated to instances by a
    // covariance-gated 1-to-1 (cov from the stabiliser posterior), a persistently-unexplained mask
    // spawns a new chair, and (if death_enabled) an unobserved instance is retired. Mirrors table.
