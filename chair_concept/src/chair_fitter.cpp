...
void ChairFitter::log_ai2_csv(const ChairInstance& inst, int npts, float R, bool gated, float energy)
{
    ...
    if (not ai2_csv_.is_open())
    {
        ...
        ai2_csv_ << "cycle,node,npts,gated,energy,fe_baseline,fe_surprise,R,motion_var,depth_var,trunc_frac,range,obliquity_cos,clutter_frac,"
                 ...
                 << "fixated,centroid_r,ego_lin,ego_ang,oblq_eff,nbv_gain_raw,nbv_gain_pub,flip1,flip2,flip3,p_mode_max,view_spent_max\n";
    }
...
float ChairFitter::los_occlusion(const ChairInstance& inst) const
{
    ...
    // The cone geometry is SHARED (common/occlusion). It returns a STRENGTH in [0,1], not a verdict, because
    // occlusion is a reason to trust absence LESS, never a licence to ignore it forever (CONCEPT_AGENT_
    // INVARIANTS, mistake II: a gate must fail to HOLD, but must not hold unconditionally and for ever).
    // This agent's hand-written version is where that shape came from; it now lives in common/ for everyone.
    const auto blocks = [&](const Eigen::Vector3f& Cs, float half, float rs) -> float
    { return rc::occlusion::cone_occlusion(O, dc, rc, Cs, half, rs, margin); };
    ...
    if (mask_ingestor_)
    {
        ...
        if (pkt.valid)
            for (const auto& sl : pkt.slices)
            {
                ...
                const float half = rc::occlusion::across_ray_half_extent(O, sl.centroid,
                                                                          sl.bbox_min, sl.bbox_max);
...
// Delegates to the SHARED bank (common/support_bank via chair_support_bank.h). This agent's own answer
// is the EXTENT + the two vertical allowances, and nothing else — see chair_support_bank.h::extent_of.
