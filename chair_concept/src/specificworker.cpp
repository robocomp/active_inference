...
void SpecificWorker::refresh_belief_strip()
{
    ...
    rc::dash::fill_strip(belief_strip_, fitter_->instances(), *G,
        ...
        {
            // exist_logodds is NaN until the existence channel initialises — leave p_exists NaN in
            // that case (the widget reads NaN as "no existence belief yet") rather than inventing 0.5.
            if (std::isfinite(inst.exist_logodds))
                r.p_exists = 1.0f / (1.0f + std::exp(-inst.exist_logodds));
            rc::dash::fill_certainty(r, inst.ai2_belief.covariance_reported(), rc::kChairDofs);
        });
}
    ...
    if (not tracks.empty() and mask_ingestor_)
    {
        ...
        if (not dets_p.empty())
        {
            ...
            for (const auto& t : tracks)
                trefs.push_back({t.id, t.xy, 0.30f});
...
{
    // The cycle around the planner call is SHARED (common/epistemic_step). ★For chair the important part of
    ...
    hooks.compute = [&]() -> std::optional<rc::EpistemicProposal>
    {
        ...
        if (not prop.valid or not prop.is_finite())
            return std::nullopt;
        // A BEARING-ONLY hypothesis has no depth: the target is the bearing itself, and the contract below
        // is Orient rather than Servo.
    ...
    rc::epistemic::step(inst, *G,
                        {.cooldown_cycles = cfg_.epistemic_cooldown_cycles,
                         .orient_mode     = inst.is_bearing_hypothesis},
                        hooks);
}
