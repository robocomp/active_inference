...
JAR="$1"; cd "$(dirname "$0")"
printf "%-7s %-7s %-7s %-7s | %-9s %-9s %s\n" LATCH LEVEL REFUSE REARM deadlock Progress "verdict"
printf -- "------- ------- ------- ------- + --------- --------- -------\n"
for LA in TRUE FALSE; do for L in TRUE FALSE; do for R in TRUE FALSE; do for A in TRUE FALSE; do
  cat > cfg.cfg <<CFG
SPECIFICATION Spec
CONSTANTS LevelTriggered = $L ConsumerRefuses = $R RearmUnchanged = $A LatchOnDecide = $LA
PROPERTY Progress
INVARIANT TypeOK
CFG
