 ...
 * door_affordance.h
 *
 * Manages the lifecycle of an "affordance" DSR node that hangs from a door
 * node via a "has_intention" edge.  The node advertises the robot pose that
 * maximises epistemic value for this door, and tracks the mission-controller's
 ...
 * DSR edge type  : has_intention   (door_node → affordance_node)
 *
...
namespace rc {
class DoorAffordance
{
public:
    enum class State { idle, pending, executing, satisfied, aborted };

    DoorAffordance() = default;

    /// Late initialisation — call once after the door DSR node is known.
...
private:
    ...
    uint64_t    door_node_id_   = 0;
    std::string door_node_name_;
