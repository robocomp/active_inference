...
void SpecificWorker::cleanup_owned_nodes()
{
    ...
    if (G and fitter_)
    {
        ...
        std::vector<std::pair<std::uint64_t, std::string>> door_nodes;

        affordance_nodes.reserve(fitter_->instances().size());
        door_nodes.reserve(fitter_->instances().size());
        ...
        for (const auto& [id, inst] : fitter_->instances())
        {
            ...
            // would otherwise be left orphaned (its parent door node is deleted
            // below), leaving a dangling door_afford node + edge in the graph.
            if (inst.affordance.node_id() != 0)
                affordance_nodes.emplace_back(inst.affordance.node_id(), inst.node_name);
            door_nodes.emplace_back(id, inst.node_name);
        }

        // Remove affordance nodes first (children of door nodes)
        for (const auto& [affordance_id, node_name] : affordance_nodes)
        {
            G->delete_node(affordance_id);
            qInfo() << "[door_concept] removed affordance node for"
                    << QString::fromStdString(node_name);
        }

        // Remove door nodes themselves
        for (const auto& [door_id, node_name] : door_nodes)
        {
            G->delete_node(door_id);
            qInfo() << "[door_concept] removed door node"
                    << QString::fromStdString(node_name);
        }
    }
