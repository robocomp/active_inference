/*
 * hood_belief.cpp  —  AI2 hood belief implementation (single floor-anchored box)
 *
 * The hood-specific model hooks behind HoodBelief: ONE box SDF (floor-anchored, z∈[0,H]),
...
float HoodBelief::sdf_box(const Eigen::Vector3f& p, const HoodBeliefState& s) const
{
...
float HoodBelief::mixture_unnormalized(const Eigen::Vector3f& p, const HoodBeliefState& s,
...
float HoodBelief::wall_component_frac() const
{
...
float HoodBelief::wall_component(const Eigen::Vector3f& p, float R, float pw) const
{
...
std::array<float, 3> HoodBelief::responsibilities(const Eigen::Vector3f& p,
                                                          const HoodBeliefState& s, float R) const
{
    std::array<float, 3> u{};
...
float HoodBelief::mean_energy(const std::vector<Eigen::Vector3f>& pts,
                                      const HoodBeliefState& s, float R) const
{
...
float HoodBelief::mean_energy_no_box(const std::vector<Eigen::Vector3f>& pts, float R) const
{
    if (pts.empty()) return 0.0f;
    const float eps = std::clamp(params_.clutter_frac, 0.0f, 0.99f);
    const float pw  = wall_component_frac();
...
Eigen::Matrix<float, 6, 1> HoodBelief::sdf_jacobian(const Eigen::Vector3f& p,
                                                            const HoodBeliefState& s, int prim) const
{
    ...
    for (int j = 0; j < 6; ++j)
    {
        ...
        vp(j) += e; vm(j) -= e;
        const float slope = (sdf_prim(p, HoodBeliefState::from_vec(vp), prim) -
                             sdf_prim(p, HoodBeliefState::from_vec(vm), prim)) / (2.0f * e);
...
void HoodBelief::apply_constraints(HoodBeliefState& s) const
{
...
Eigen::Matrix<float, 6, 1> HoodBelief::prior_cov_diag() const
{
...
Eigen::Matrix<float, 6, 1> HoodBelief::process_noise_diag() const
{
...
void HoodBelief::update_volatility(const Eigen::Matrix<float, 6, 1>& dtheta)
{
...
Eigen::Matrix<float, 6, 1> HoodBelief::retention_frames() const
{
...
Eigen::Matrix<float, 6, 1> HoodBelief::common_mode_inv_diag(const HoodFrame& frame) const
{
...
Eigen::Vector2f HoodBelief::back_centre(const HoodBeliefState& s) const
{
...
void HoodBelief::accumulate_wall(const HoodBeliefState& s, const HoodFrame& f,
                                         Eigen::Matrix<float, 6, 6>& Id, Eigen::Matrix<float, 6, 1>& bd) const
{
    ...
    const auto par_of = [&](const HoodBeliefState& st)   // width axis (local +X) · wall normal
    ...
    for (int k = 0; k < 6; ++k)
    {
        ...
        vp(k) += eps; vm(k) -= eps;
        const auto sp = HoodBeliefState::from_vec(vp), sm = HoodBeliefState::from_vec(vm);
...
void HoodBelief::accumulate_extra(const HoodBeliefState& s, const HoodFrame& f,
                                          Eigen::Matrix<float, 6, 6>& Id, Eigen::Matrix<float, 6, 1>& bd) const
{
    ...
    if (f.wall.ok and params_.wall_no_cross_precision > 0.0f)
    {
        ...
        if (residual < 0.0f)
        {
            ...
            for (int k = 0; k < 6; ++k)
            {
                Eigen::Matrix<float, 6, 1> vp = v, vm = v;
                vp(k) += eps; vm(k) -= eps;
                Jg(k) = std::clamp((gap_of(HoodBeliefState::from_vec(vp))
                                  - gap_of(HoodBeliefState::from_vec(vm))) / (2.0f * eps), -g, g);
            }
...
FootprintMoment HoodBelief::footprint_moment(const std::vector<Eigen::Vector3f>& pts,
...
std::array<bool, 4> HoodBelief::allowed_modes() const
{
...
//
// A hood stands with its BACK against a wall, so its door faces the room — it physically cannot face
...
// nothing at all. See [[hood-door-clearance-prior]].
//
...
std::array<float, 4> HoodBelief::door_clearance_logprior() const
{
...
bool HoodBelief::resolve_front_geometric()
{
...
void HoodBelief::apply_mode_rotation(int k)
{
    constexpr float kHalfPi = 0.5f * static_cast<float>(M_PI);
    const auto wrap = [](float a) { return std::remainder(a, 2.0f * static_cast<float>(M_PI)); };
...
bool HoodBelief::resolve_front(const FrontCue& cue, float evidence_weight)
{
    constexpr float kHalfPi    = 0.5f * static_cast<float>(M_PI);
    ...
    // is unconditional. See [[hood-door-mode-lock]].
    for (int k = 0; k < 4; ++k)
    {
        const float psi = wrap(state_.yaw + static_cast<float>(k) * kHalfPi);
...
std::array<float, 4> HoodBelief::front_posterior() const
{
    const std::array<bool, 4> allowed = allowed_modes();
...
float HoodBelief::front_confidence() const
{
    const auto p = front_posterior();
    return *std::max_element(p.begin(), p.end());
}

int HoodBelief::front_mode() const
{
    const auto p = front_posterior();
...
float HoodBelief::yaw_marginal_var() const
{
    constexpr float kHalfPi = 0.5f * static_cast<float>(M_PI);
    const auto wrap = [](float a) { return std::remainder(a, 2.0f * static_cast<float>(M_PI)); };
    const auto p = front_posterior();
...
Eigen::Matrix<float, 6, 6> HoodBelief::covariance_reported() const
{
...
float HoodBelief::fridge_log_evidence_ratio(const HoodBeliefState& s,
                                                    const HoodBeliefParams& p)
{
...
std::vector<float> HoodBelief::singleton_existence_deltas(const std::vector<float>& plaus_evidence,
...
bool HoodBelief::self_test()
{
    ...
    HoodBeliefParams P;
    const HoodBeliefState gt{0.20f, -0.30f, 1.75f, 0.70f, 0.60f, 0.30f};   // ground truth (w≠h)
    ...
    HoodBeliefState s0{0.0f, 0.0f, 1.4f, 0.5f, 0.5f, 0.15f};
    ...
    HoodBelief b(s0, P);
    HoodFrame frame; frame.points = pts;
    ...
    std::printf("HoodBelief::self_test [full-cloud] %s  cx=%.3f cy=%.3f H=%.3f w=%.3f h=%.3f yaw=%.3f (gt %.3f)\n",
    ...
    const auto front_only_cloud = [&](const HoodBeliefState& g)
    {
    ...
    const HoodBeliefState gtw{0.50f, 0.30f, 1.70f, 0.60f, 0.60f, 0.0f};   // back face flush at y=0
    HoodFrame fw;
    ...
    HoodBeliefState sw{0.50f, 0.175f, 1.70f, 0.60f, 0.85f, 0.0f};
    HoodBelief bw(sw, P);
    ...
    std::printf("HoodBelief::self_test [wall-flush partial] %s  cy=%.3f h=%.3f w=%.3f back_gap=%.3f λ=%.1f\n",
    ...
    HoodBeliefState sf{0.50f, 0.30f, 1.70f, 0.60f, 0.60f, 0.0f};
    HoodBelief bf(sf, P);
    ...
    std::printf("HoodBelief::self_test [free-standing]     %s  cy=%.3f h=%.3f λ=%.3f\n",
    ...
    const HoodBeliefState gtc{0.50f, -0.10f, 1.70f, 0.60f, 0.60f, 0.0f};   // front face at y=+0.20
    HoodFrame fc;
    ...
    const HoodBeliefState sc{0.50f, -0.10f, 1.70f, 0.60f, 0.60f, 0.0f};    // seed already crossing (back at −0.40)

    HoodBeliefParams Poff = P; Poff.wall_no_cross_precision = 0.0f;         // constraint OFF (baseline)
    HoodBelief boff(sc, Poff); boff.set_room_interior({0.5f, 5.0f});
    for (int it = 0; it < 80; ++it) boff.update(fc);
    const float back_off = boff.back_centre(boff.state()).y();                      // expect strongly negative (crossed)

    HoodBeliefParams Pon = P; Pon.wall_no_cross_precision = 100000.0f;      // firm, to demonstrate the null
    HoodBelief bon(sc, Pon); bon.set_room_interior({0.5f, 5.0f});
    ...
    const HoodBeliefState gtf{0.50f, 0.60f, 1.70f, 0.60f, 0.60f, 0.0f};     // front face at y=+0.90, back at +0.30
    HoodFrame fcf; fcf.points = front_only_cloud(gtf);
    ...
    HoodBelief bfront(gtf, Pon); bfront.set_room_interior({0.5f, 5.0f});
    ...
    std::printf("HoodBelief::self_test [wall no-cross]      %s  back_off=%.3f back_on=%.3f back_front=%.3f\n",
    ...
    const auto door_bearing = [&](const HoodBelief& b)   // room bearing the believed door (local −Y) faces
    { const float y = b.state().yaw; return std::atan2(-std::cos(y), std::sin(y)); };

    HoodBeliefState sq{0.0f, 0.0f, 1.70f, 0.60f, 0.60f, 0.0f};   // square footprint, door believed at −π/2
    HoodBelief bd(sq, P);
    ...
    std::printf("HoodBelief::self_test [front-resolver]     %s  door_bearing=%.3f (gt %.3f) σyaw %.2f→%.2f p=%.2f\n",
    ...
    const HoodBeliefState proper{0.0f, 0.0f, 1.75f, 0.60f, 0.60f, 0.0f};   // square + tall
    const HoodBeliefState ordinary{0.0f, 0.0f, 1.52f, 0.77f, 0.63f, 0.0f}; // a real, slightly wide fridge
    const HoodBeliefState wrongish{0.0f, 0.0f, 0.70f, 1.20f, 0.40f, 0.0f}; // elongated + short (the mis-detection)
    ...
    std::printf("HoodBelief::self_test [shape-LLR]          %s  proper=%+.2f ordinary=%+.2f wrongish=%+.2f\n",
    ...
    const auto step_of = [&](const HoodBeliefState& st)
    { return std::clamp(fridge_log_evidence_ratio(st, P), -kClamp, kClamp); };
    const auto accumulate = [&](const HoodBeliefState& st, int frames, float ev0) {
        float ev = ev0;
    ...
    std::printf("HoodBelief::self_test [plaus-evidence]     %s  ev_proper=%.2f ev_ordinary=%.2f ev_wrong=%.2f ev_after_bad=%.2f\n",
    ...
    std::printf("HoodBelief::self_test [singleton]          %s  L_strong=%.2f L_weak=%.2f (removeL=%.2f)\n",
    ...
    const auto fit_fe = [&](float Hgt) {
        HoodBeliefState s0{0.0f, 0.0f, Hgt, 0.60f, 0.60f, 0.0f};
        HoodBelief bb(s0, Ph);
        HoodFrame fr; fr.points = box_cloud(Hgt);
    ...
    std::printf("HoodBelief::self_test [short-height prior] %s  FE(0.70m)=%.3f > FE(1.70m)=%.3f\n",
    ...
    // The discriminating case from fridge_1.png: YOLO puts a "hood" box on a flat piece of WALL. Both
    ...
    {
        HoodBeliefParams Pw = P;
        Pw.wall_explain_frac = 0.25f; Pw.wall_explain_sigma_m = 0.05f;
        HoodBelief bw(HoodBeliefState{0.0f, 0.0f, 1.90f, 0.60f, 0.60f, 0.0f}, Pw);
        ...
        const HoodBeliefState box_on_wall  {0.0f, 0.0f, 1.90f, 0.60f, 0.60f, 0.0f};   // centre in the wall
        const HoodBeliefState box_on_floor {0.30f, 0.0f, 1.90f, 0.60f, 0.60f, 0.0f};  // front face at x=0.60
        ...
        std::printf("HoodBelief::self_test [wall explain-away] %s  gain wall=%+.2f fridge=%+.2f | wall-resp %.2f vs %.2f\n",
    ...
    bool ok_lock = false;
    {
        HoodBelief bl(HoodBeliefState{0.0f, 0.0f, 1.90f, 0.60f, 0.60f, 0.0f}, P);
        ...
        std::printf("HoodBelief::self_test [door-mode lock]    %s  saturated conf=%.3f → %s, turned %.1f°\n",
    ...
    bool ok_vol = false;
    {
        ...
        const auto run = [&](float step_m, int frames) {
            HoodBelief b(HoodBeliefState{0.0f, 0.0f, 1.90f, 0.60f, 0.60f, 0.0f}, Pv);
        ...
        HoodBelief b_static = run(0.0002f, 600);
        // (2) MOVED: the fridge is shoved — 4 cm steps.
        HoodBelief b_moved  = run(0.04f, 600);
        ...
        std::printf("HoodBelief::self_test [volatility]        %s  omega0=%.2f | static->%.2f (retention x%.1f) | moved->%.2f (retention x%.2f)\n",
