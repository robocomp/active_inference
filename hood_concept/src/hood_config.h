/*
 * hood_config.h  —  plain-data configuration for the hood_concept agent + its ConfigLoader loader.
 *
 ...
 * (hood_config.cpp) overrides from the RoboComp TOML when the key is present.
 */
...
struct HoodConfig
{
    ...
    // Forwarded to HoodModelParams.sigma_obs: a mask point within TOP_THICKNESS + sigma_obs below the top
    ...
    // ── AI2 belief (HOOD.md) — full-covariance recursive filter ──────────────────────────
    ...
    // WALL as a competing per-point explanation (explaining away — see HoodBeliefParams). π_wall and the
    ...
    // move the GEOMETRY MEAN: a moving frame confirms but can't reshape/reposition/rotate the hood; geometry
    ...
    float ai2_range_noise_size_per_m = 0.08f;  // SIZE (w,h,H) common-mode std growth (m per m of range): a distant
                                               // mask can't reshape/inflate a converged hood (freezes geometry afar)
    ...
    // THRESHOLD (outlier step guard): a static hood cannot physically move this far in one frame, so a GN
    ...
    // large mask grows to explain it (fixes hood_1.png). 0 = OFF.
    ...
    // the hoodtop boundary past the empty crossing. Coverage occupies where masked; free-space vacates where
    ...
    // ── Footprint SECOND-MOMENT factor (hood_belief.h) ───────────────────────────────────────────
    ...
    // (hoods_5.png). Also seeds (w,h,yaw) at birth. yaw pull scales with extent anisotropy; capped by the
    ...
    // Baseline EMA rates: DOWN fast (consolidate a better fit), UP slow (a sustained mismatch — the hood
    ...
    // the per-point obliquity yaw cap (kObliquityYawGain in hood_fitter.cpp) were validated live 2026-07-11 and
    ...
    // footprint_moment_completeness_gain / _min_completeness: forwarded to HoodBeliefParams — inflate the moment
    ...
    // winning face's bearing into the belief's sequential-Bayes door-mode resolver (HoodBelief::resolve_front).
    ...
    // Each cycle carve the LiDAR sweep against the hood footprint → occupancy/free-space log-odds; remove
    ...
    // when the shared rc::exist::RemovalPolicy showed hood and cabinet were the only two
    ...
    float existence_verify_surprise     = 20.0f;  // decayed go-verify surprise (un-resolvable absence) above which
                                                  // a hood is flagged wants_verification (epistemic pull, not removal)
    ...
    float existence_verify_gain         = 5.0f;   // epistemic gain (nats) a wants_verification hood gets, so the
    ...
    // never deleted. There is no leg carve: a hood has no legs.

    // ── "Is this really a fridge?" plausibility filter + soft singleton (model-evidence mis-detection reject) ─
    // A YOLO "hood" mis-detection (e.g. a ~70 cm elongated cabinet) contradicts the fridge shape priors
    ...
    // Upload the hood pose covariance onto the room→hood RT edge (rt_covariance_att, 6×6 SE3), mapped from
    ...
    // hood as an SE(2) pose landmark. OFF by default (consumer side is also flag-gated + defaults OFF).
    bool        publish_object_obs = false;      // HoodConcept.PublishObjectObs
    std::string object_obs_frame   = "body";     // HoodConcept.ObjectObsFrame (localizer base node)

    // ── Multi-instance birth/associate/death (shared rc::InstanceTracker) ─────────────────────────
    // "hood" masks are associated to instances by a covariance-gated global 1-to-1; an unexplained mask
    // spawns a new hood. There is NO death-by-occlusion: a hood is large static furniture, so a long
    // occlusion is not absence. An instance leaves only via the MERGE operator (two hoods cannot share
    // space) or the existence-removal channel (hood_existence.cpp). Keep birth_min_sep large so no two
    // hood centres can sit that close.
    ...
    // Detection-noise std R (m) added to the fit cov in the Mahalanobis gate (S = P + R²I). For a hood
    ...
    int   tracker_birth_frames      = 8;      // frames a mask must stay unexplained before spawning a hood
    ...
    float tracker_birth_min_sep_m   = 0.60f;  // a birth must be ≥ this (m) from every existing hood
    // Physical exclusion: collapse two instances whose oriented footprints overlap by ≥ this fraction of
    // the SMALLER footprint (two hoods cannot share space). 0 disables the merge.
    float tracker_merge_overlap = 0.05f;
    // Default geometry for a hood BORN from a mask, used only when no birth burst is available
    ...
    // ADMISSION on the accumulated burst. HoodBelief::candidate_plausibility is already applied
    ...
    // range roughness when deciding whether an unassigned ricoh bearing already matches a known hood.
    float ricoh_attention_angle_margin_rad = 0.05f;  // extra angular tolerance on the hood's angular half-size
    ...
    bool  ricoh_confirm_enabled        = false;   // HoodConcept.RicohConfirmEnabled
    ...
    float ricoh_confirm_detection_prob = 0.60f;   // HoodConcept.RicohConfirmDetectionProb
    float ricoh_confirm_clutter_prob   = 0.15f;   // HoodConcept.RicohConfirmClutterProb

};

// Fill a HoodConfig from a RoboComp ConfigLoader (all keys optional, defaults above).
HoodConfig load_hood_config(const ConfigLoader& cfg);

// Cross-check common/concept_manifest/hood.concept.toml against the priors etc/config.toml actually
