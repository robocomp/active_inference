/*
 * hood_existence.cpp — see hood_existence.h. Evidence-based hood removal (existence log-odds + debounce).
 */

#include "hood_existence.h"
                                       ...
                                       const std::function<void(std::uint64_t, const HoodInstance&)>& on_remove)
{
    ...
    for (auto& [id, inst] : fitter.instances())
    {
        ...
        // per predicted-VISIBLE pixel, against the current YOLO foreground. Lit by a "hood" mask ⇒ occupancy
        // (holds L up); lit by NOTHING ⇒ predicted-but-absent (the "gone" signal, present EVEN WITH NO YOLO MASK);
        // lit by a NON-hood mask ⇒ occluded ⇒ HOLD (never false absence behind a nearer object). n_detectable==0
        ...
        if (fresh_masks)
        {
            ...
            if (sil.n_detectable > 0)
            {
                ...
                // frame it is not gone, so suppress silhouette ABSENCE — a ZED false-negative, or a hood seen
                ...
                // must make the robot GO VERIFY, never delete a hood it never properly looked at.
                ...
                // hood IS detectable up close (hood_1), so obliquity would wrongly block legitimate removal.
                ...
                // a phantom hood standing INSIDE the dining set collected occupancy from the real
        ...
        // LiDAR occupancy carve of the WHOLE SOLID BOX z∈[0, H] — LiDAR clock. A hood is one solid
        // floor-anchored cuboid (hood_model.cpp), so every ring that reaches it strikes real surface at
        ...
        // depending on whether a ring happened to graze a fake leg. See [[hood-table-geometry-churn]].
        ...
        if (sweep)
        {
            ...
            if (ev.n_reached > 0)                                                    // 0 ⇒ not probed ⇒ HOLD
            {
                ...
                // ran. Live: phantom hood_2 accrued confident camera absence on 2885 of 3255 cycles
        ...
        // cycles) regardless of the two sensors' rates. A transient hiccup still can't delete a real hood.
        if (integrated)
        {
            ...
            // exactly where a spurious LiDAR-absence carve accrues and removes a real hood. Emit a row here for
    ...
    {
        std::print("hood_concept: [existence] removing hood id={} — free-space evidence (empty volume)\n", id);
