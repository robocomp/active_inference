 ...
 * support points into on-surface vs off-surface sets), inference (support-bank ingest + one recursive HoodBelief
 * update with the mask-motion channel as the observation precision R / bias gate, written back into inst.model),
 * and the hood-owned support-point memory (ownership gate + FNV cell keys). Collaborates with MaskIngestor (masks),
