...
#include "hood_model.h"        // HoodModel / HoodState
#include "hood_belief.h"       // AI2 full-covariance belief (HOOD.md)
...
struct HoodInstance
{
    ...
    float dbg_lidar_topz_m   = -1.0f;   // mean z of the HIGHEST 20% of selected returns ≈ observed hoodtop z
    ...
    // obj_obs_robot_cov=[Rxx,Ryy,Rxy]. See HOOD_TRIANGULATION.md.
    Eigen::Matrix2f obs_robot_cov = Eigen::Matrix2f::Identity();
    bool obs_robot_valid = false;
    int  processed_cycles = 0;        // per-hood compute cycles for log throttling
    // Tracker's gated mask assignments for THIS frame (indices into the masks packet slices). With
    // multi-detection fusion a hood can be seen by BOTH the ZED and the ricoh-360 masks in one cycle, so
    // this is a LIST — process_hood_node runs one belief update per slice (sequential = joint likelihood,
    ...
    HoodState prev_conv_state{};      // accepted state at the previous cycle (for state-delta convergence)
    ...
    int  epistemic_cooldown  = 0;   // cycles remaining before a satisfied hood may re-arm
    ...
    // hood stops jittering the retina mesh. Mirrors bottle_concept's last_pub_* publish gate.
    ...
    // Trace of the last RT-edge covariance published, so a stationary-but-still-tightening hood refreshes its
    ...
    // Detection aliveness: how recently YOLO produced a "hood" mask for this instance, and the confidence
    // of the last one. The controller hill-climbs these during the micro-search.
    int   frames_since_detection = 100000;   // cycles since last fresh hood mask (0 = just detected)
    float last_mask_confidence   = 0.0f;     // YOLO confidence of the last hood detection
    bool  detection_alive        = false;    // frames_since_detection < threshold

    // ── Existence log-odds (removal) ──────────────────────────────────────────────────────────────
    // LiDAR free-space carve + mask-absence accrue negative evidence when the hood is demonstrably gone.
    ...
    // defect that deleted well-fitted fridges with zero new data. See [[hood-table-geometry-churn]].
    ...
    // CANNOT resolve the hood (far / peripheral / edge-on) does NOT vote removal — it raises this decayed
    ...
    // hood settles to (slow EMA; tracks DOWN fast to consolidate a better fit, UP slow so a SUSTAINED rise — the
    // world changed, e.g. the hood was moved — shows as surprise before the baseline accepts it). fe_surprise =
    // the smoothed positive gap (FE − baseline): ~0 when the belief matches the data, spikes when the hood moves
    // and decays as the robot re-observes and the fit re-converges. This is the signal that will raise the hood's
    ...
    // hoodtop view is → foreshortened footprint) and completeness (observed vs believed footprint area).
    float dbg_yaw_pre       = 0.0f;   // belief yaw at cycle start, before the update (rad)
    float dbg_obliquity_cos = 1.0f;   // |cos(incidence)| of camera→hood ray vs hoodtop normal +z (1=top-down, →0 grazing)
    ...
    // ── Appearance FRONT (door) detection state (HoodProjection::detect_front + belief.resolve_front) ─────
