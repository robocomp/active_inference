/*
 * hood_projection.cpp — camera-projection unit for hood_concept (extracted from HoodFitter).
 *
...
// the quarter-turn is never resolved and the fridge renders rotated 90°. See [[hood-square-yaw-door-cue]].
...
// faces and emitted nothing for a head-on, wall-flush fridge (etc/fridge_1.png: `hood 0.95`, one face,
...
std::optional<Eigen::Matrix4d> HoodProjection::room_T_zed_matrix(std::uint64_t pose_ts_ms) const
{
    if (not inner_eigen_)
        return std::nullopt;
    ...
    static rc::rtprobe::Probe rt_probe{"hood_projection room<-body"};
...
void HoodProjection::compute_projected_roi(HoodInstance& inst)
{
...
// camera is above it) onto the image and, over the predicted-visible pixels, splits into: lit by a "hood"
// mask (occupancy), lit by a non-hood mask (occlusion → HOLD), or lit by nothing (absence — the KEY
// "gone" signal that fires even when NO YOLO mask does this frame).
SilhouetteExistence HoodProjection::compute_silhouette_existence(const HoodInstance& inst)
{
    ...
    // Hashed pixel-cell coverage of the current YOLO foreground, split hood (occupancy) vs other (occluder).
    ...
    std::unordered_set<std::int64_t> hood_cells, occluder_cells;
    for (const auto& sl : pkt.slices)
    {
        ...
        auto& dst = (sl.label == "hood") ? hood_cells : occluder_cells;
    ...
    // the sample is behind a wall. See [[hood-silhouette-line-of-sight]].
    ...
    // sample, far outside the skip. See [[hood-silhouette-line-of-sight]].
    ...
    // sample lit by a "hood" mask is occupancy; lit by nothing is ABSENCE (the "gone" signal that fires even
    // with NO YOLO mask); lit by a non-hood mask is OCCLUDED (a nearer object hides the point) ⇒ HOLD.
    ...
    const auto classify = [&](float lx, float ly, float lz)
    {
        ...
        if (occluder_cells.contains(k) and not hood_cells.contains(k))
        ...
        if (hood_cells.contains(k)) out.e_occ  += 1.0f;              // still there
        else                         out.e_free += 1.0f;              // predicted-but-absent
    };

    // A hood is a SOLID floor-anchored cuboid (hood_model.cpp), so its predicted silhouette is
    ...
    // scored socc=0 / sfree=640 → L pinned at −4 → removed, then re-born. See [[hood-table-geometry-churn]].
    //
...
std::optional<FrontCue> HoodProjection::detect_front(const HoodState& s, const cv::Mat& rgb,
...
bool HoodProjection::self_test()
{
    ...
    std::printf("HoodProjection::self_test [door-ness] %s  lined=%.2f plain=%.2f (ratio %.1f)\n",
    ...
    std::printf("HoodProjection::self_test [door-seam] %s  front=%.2f (conf %.2f) back=%.2f (conf %.2f) vert-band=%.2f\n",
