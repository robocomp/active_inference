/*
 * hood_voxel_bank.h — the hood-owned voxel memory (extracted from HoodFitter).
 *
 * Header-only free functions in rc::voxel_bank that maintain each instance's persistent point bank: key()
 * (FNV-1a quantised voxel key), owned_by_hood() (XY-radius + height ownership gate), and ingest() (insert
 * this cycle's candidate/residual points, cap-bounded, ownership-gated, de-duplicated by key). Depends only
 * on rc::HoodInstance and rc::HoodConfig, so it never pulls in HoodFitter's nested observation type.
 */
...
// True iff a point belongs to THIS hood's voxel bank (ownership gate rejecting foreign/floor points).
//
// Two THRESHOLDS, both physical, keep a neighbouring hood's or the floor's points out of the bank:
// an XY radius (hood half-diagonal + config margin, floored at 1 m so a tiny model still owns nearby
// returns) and a vertical band [z_min, hood_height + config margin].
inline bool owned_by_hood(const HoodInstance& inst, const Eigen::Vector3f& point, const HoodConfig& cfg)
{
    const auto& s = inst.model.state();

    // XY ownership gate: hood-centered radius with a configurable margin.
...
inline void ingest(HoodInstance& inst,
    ...
    auto ingest_pts = [&](const std::vector<Eigen::Vector3f>& src)
    {
        for (const auto& p : src)
        {
            if (inst.voxel_bank_pts.size() >= max_points)
                break;
            if (not owned_by_hood(inst, p, cfg))
            {
                ++rejected_foreign;
                continue;
            }
