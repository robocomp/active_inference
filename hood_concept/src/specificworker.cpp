...
void SpecificWorker::initialize()
{
    ...
    lidar_ingestor_ = std::make_unique<rc::HoodLidarIngestor>(G, inner_eigen_.get(), cfg_);
...
void SpecificWorker::log_birth_surprise()
{
    ...
    for (const auto& c : cands) if (not c.covered_by_hood) ++n_birth;
    ...
    if (birth_surprise_csv_.is_open())
    {
        ...
        for (const auto& c : cands)
            birth_surprise_csv_ << cyc << ',' << r++ << ',' << c.cx << ',' << c.cy << ',' << c.cells << ','
                                ...
                                << ',' << (c.covered_by_hood ? 1 : 0) << ',' << hoods.size() << ','
    ...
    if (n_birth > 0 and (ev_g_.births > 0 or (birth_surprise_log_ctr_++ % 20) == 0))
    {
        ...
        for (const auto& c : cands) if (not c.covered_by_hood) { top = &c; break; }   // cands sorted by mass
        if (top)
...
void SpecificWorker::step_epistemic(rc::HoodInstance& inst, DSR::Node& node)
{
    ...
    // Controller-completion hold (anti-churn): the hood affordance completes on a weak detection
    // (contract goal conf≥0.20), which fires almost instantly — before ΔH has decayed. Start a short
    // cooldown so we don't re-offer a just-completed hood while its gain is still high. We do NOT
    ...
    // in, no node churn. During the post-completion hold the gain is forced to 0 so a just-finished hood
    // isn't re-claimed before its belief has settled.
    if (inst.epistemic_cooldown > 0)
        prop.epistemic_gain = 0.0f;

    // Verification pull (active inference): a hood whose predicted absence could NOT be resolved from recent
    ...
    // deletes a hood the robot hasn't properly looked at — it sends the robot to look. Overrides the cooldown
