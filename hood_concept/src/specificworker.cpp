      ...
      startup_check_flag(startup_check)
{
    ...
    // default silently.
    int period = 0;
    rc::cfg::Reader(configLoader, "hood_concept").req("Period.Compute", period,
...
void SpecificWorker::initialize()
{
    ...
    // ARMS the unread sweep; check_registry_complete.sh hood_concept is the grep that keeps the claim
    // honest. Re-run it whenever a config read is added.
    rc::cfg::exempt_generated_prefixes();
    rc::cfg::registry().declare_complete("hood_concept");
    rc::cfg::Reader(configLoader, "hood_concept").publish("etc/config_effective.csv");
}
...
void SpecificWorker::log_birth_surprise()
{
    ...
    // Believed hood footprints (room frame) — a region under one is already explained, NOT a birth. This is
    // the one genuinely per-object step: a hood's two footprint extents. Everything after it — both CSVs,
    ...
    birth_surprise_log_.write(residual_field_, footprints, last_hood_dets_xy_,
