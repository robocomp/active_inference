...
/**
 * specificworker.h — hood_concept agent (orchestration + Qt/DSR glue).
 *
 * Per compute() cycle: ingest ZED YOLO "hood" masks → InstanceTracker (birth / associate / merge) →
 * process_hood_node (one AI2 HoodBelief update per assigned slice) → publish the fit back to DSR (RT edge +
 * dims + mesh + covariance) and emit epistemic proposals when a hood stays under-observed. Ricoh-360
 * detections are bearing-only PERIPHERAL ATTENTION (they never birth or fit). Also owns the two standalone
 * top-level windows (belief dashboard + evidence monitor). The fit core is rc::HoodFitter, perception
 * rc::MaskIngestor, DSR I/O rc::HoodSceneGraph. See HOOD.md for the belief/fit core.
 */
...
private:
    // The fit (observe/infer/belief/support-bank) lives in rc::HoodFitter; perception in
    // rc::MaskIngestor; DSR I/O in rc::HoodSceneGraph. The worker keeps orchestration + the
    // post-fit epistemic/affordance/Qt-diagnostics steps.
    using HoodObservation = rc::HoodFitter::HoodObservation;
    ...
    void process_hood_node(const DSR::Node& node);
    void publish_hood_cycle(rc::HoodInstance& inst,
                             const DSR::Node& node,
                             const HoodObservation& observation,
                             float free_energy);
    bool assess_hood_state(rc::HoodInstance& inst, uint64_t node_id, float free_energy);
    void publish_hood_diagnostics(const rc::HoodInstance& inst,
                                   const HoodObservation& observation,
                                   float free_energy);
    void publish_hood_intentions(rc::HoodInstance& inst,
                                  uint64_t node_id,
                                  const HoodObservation& observation,
                                  float free_energy);
    void step_convergence(rc::HoodInstance& inst, DSR::Node& node, float free_energy);
    void step_epistemic(rc::HoodInstance& inst, DSR::Node& node);
    void trigger_graph_layout_twopi();   // injected into HoodSceneGraph as the relayout callback

    // Multi-instance birth/associate/merge (shared rc::InstanceTracker; the only instance-lifecycle path).
    // Associates "hood" masks to instances, spawns a hood from an unexplained mask, merges overlaps.
    ...
    // Physical-exclusion invariant: two hoods cannot share space. Collapse any pair of instances whose
    ...
    void remove_stale_affordance_nodes();   // sweep affordances parented to a hood (start + exit)
    void remove_owned_hood_nodes();        // startup stale-sweep of "hood*" nodes (mirrors bottle)
    ...
    std::unordered_set<std::string> ts_known_hoods_;   // node_names with live timeseries series (for pruning)
    void prune_dead_series();   // drop timeseries series for hoods removed from the graph (periodic, in compute)
    ...
    std::vector<Eigen::Vector2f> last_hood_dets_xy_;   // this cycle's ZED "hood" detection centroids (room frame)
    ...
    // matures a phantom in well under a second. See [[hood-birth-tightening]].
    ...
    FPSCounter fps_counter_;   // per-cycle "[hood_concept Compute] Period/Fps/cpu%/mem" heartbeat (std::cout)
