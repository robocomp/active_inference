 ...
 *  - merge_overlapping_instances : physical-exclusion collapse of two instances fitted to one hood,
...
#include "hood_geometry.h"   // rc::geom::footprint_overlap_ratio
...
// Data-driven multi-instance lifecycle (mirrors bottle_concept::run_instance_tracker). Hoods are large
// static furniture, so birth_min_sep is wide, death is off, and overlaps merge. The only path that
// creates/associates hood instances (the prior-scaffold + greedy nearest-mask were removed in Stage 2).
// Collapse instances whose footprints overlap (same physical hood fitted twice): keep the one with more
...
void SpecificWorker::merge_overlapping_instances()
{
    ...
    for (std::size_t i = 0; i < ids.size(); ++i)
    {
        ...
        for (std::size_t j = i + 1; j < ids.size(); ++j)
        {
            ...
            std::print("hood_concept: [tracker] MERGE id={} into id={} (footprint overlap {:.2f})\n",
                       drop, keep, ratio);
...
// detections from this frame's ZED "hood" slices, then apply the result (death / associate / birth). Ricoh
// slices are excluded here (bearing-only). This is the ONLY path that creates/associates hood instances.
void SpecificWorker::run_instance_tracker()
{
    ...
    tp.death_frames     = std::numeric_limits<int>::max();   // never retire by occlusion (see hood_config.h)
    tp.birth_min_sep_m  = cfg_.tracker_birth_min_sep_m;
    tp.multi_det_per_track = true;   // fuse multiple ZED slices of one hood (one belief update per slice)
    ...
    // Detections ← the current "hood" mask slices (carry the slice index for the assignment). Built EVERY
    ...
    // separately, in process_hood_node (per-instance frame_id gate), NOT here.
    ...
    if (pkt.valid)
        for (int i = 0; i < static_cast<int>(pkt.slices.size()); ++i)
        {
            ...
            if (sl.label != "hood" or sl.support_end <= sl.support_begin or not sl.may_fit_geometry())
                continue;
            ...
            // HoodBelief::candidate_plausibility.
            if (cfg_.fridge_filter_enabled)
            {
                const auto cloud = slice_cloud(pkt, i);
                const float cand_plaus = rc::HoodBelief::candidate_plausibility(cloud, plaus_p);
            ...
            // known. Fixes fridges being born OUTSIDE the layout. See [[hood-room-envelope-birth]].
            ...
            // mask equally, so the marginal `hood 0.53` blob on a wall in fridge_1.png matured exactly as
    ...
    // Snapshot the ZED hood-detection centroids (room frame) for the birth-surprise fusion probe (read at the
    // compute() tail in log_birth_surprise). Cheap; only used when cfg_.birth_surprise_probe is on.
    last_hood_dets_xy_.clear();
    for (const auto& d : dets) last_hood_dets_xy_.push_back(d.xy);
    ...
    {
        std::print("[tracker] instances={} hood_dets={} assigned={} unassigned={} births={} deaths={}\n",
    ...
    // DEATH: a hood is rigid, persistent furniture, so a long occlusion (no mask for many frames) is NOT
    // absence and must never retire it. The ONLY way a hood is removed is the MERGE operator (two hoods
    ...
    for (std::size_t k = 0; k < res.births.size(); ++k)
    {
        ...
        {
            const float plaus = rc::HoodBelief::candidate_plausibility(burst->pts, plaus_p);
...
// weaker mis-detection (elongated/short cabinet mislabelled "hood") decays and is removed. The worker
// owns this because the singleton term needs to see ALL instances at once. See HoodBelief::
...
void SpecificWorker::apply_fridge_filter()
{
    ...
    {
        std::print("hood_concept: [fridge-filter] removing id={} — implausible as a fridge "
...
// when its centroid/range is biased, so we use ONLY that: associate a ricoh bearing to an existing hood if the
// bearing falls within that hood's angular extent (its circumscribed radius over its range — a physical size,
// not a tuned gate). A ricoh bearing that NO hood explains is UNASSIGNED → an attention target ("there is
// something hood-like in that direction that the ZED hasn't confirmed; seek a ZED view there"). Never births,
...
void SpecificWorker::process_ricoh_bearings()
{
    ...
    {
        if (sl.label != "hood" or sl.depth_var <= 0.0f)          // ricoh (depth-carrying 360) detections only
            continue;
        ...
        // that biases the centroid). Foreground-gated upstream, so the median sits near the hoodtop surface.
        // Falls back to the centroid if too few points. Used for the ATTENTION target's bearing + rough range
        // ONLY — never the fit (a partial-view range is biased, so it decides WHERE TO LOOK / WHICH hood, not
        // where the hood IS; the ZED measures that). Bearing is reliable; range is indicative.
        ...
        for (const auto& [id, inst] : fitter_->instances())
        {
            ...
            const float tb   = std::atan2(dy, dx);                                  // bearing to the hood
            ...
            // Assign only if the detection matches this hood in BOTH direction AND (rough) range — so a new,
            // unconfirmed hood hiding along the SAME bearing as a known one (different range) stays UNASSIGNED
            // and still raises attention, instead of collapsing onto the known hood (bearing-only would miss it).
    ...
    if (not ricoh_attention_targets_.empty() and (rb_dbg++ % 10 == 0))
        for (const auto& t : ricoh_attention_targets_)
            std::print("[ricoh-attention] UNASSIGNED hood bearing={:+.0f}° range≈{:.1f}m conf={:.2f} → seek ZED view\n",
