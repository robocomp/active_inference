...
void SpecificWorker::merge_overlapping_instances()
{
    ...
    // fitted rectangles the same hood, and what does retiring one mean. The counter is filled by the
...
void SpecificWorker::process_ricoh_bearings()
{
    ...
    // sequence is SHARED (rc::peripheral::run_cycle). Only the track list and the confirm sink are hood's.
    ...
    const auto out = rc::peripheral::run_cycle(
        mask_ingestor_->packet(), "hood", robot_xy, tracks,
...
void SpecificWorker::log_detect_probe()
{
    ...
    // Camera height separately: elevation to a hood 2.3 m up is a first-order covariate and the body
    // frame is not where the lens is.
    ...
    for (const auto& [id, inst] : fitter_->instances())
    {
        ...
        row.ocx = s.cx; row.ocy = s.cy; row.ocz = 0.5f * (s.z0() + s.z1());
        ...
        // ★THE LABEL, and it is not det_alive. detection_alive LATCHES across frames (measured: 1 in
        // 9432/9432 hood rows, still 1 at frames_since_det=6172), so as an outcome it is degenerate and
        ...
        row.sibling = rc::probe::nearest_other_label(pkt, "hood", {s.cx, s.cy});
