...
// keyed on hood's PRIMARY input (the retina `masks` node) instead of a LiDAR media plane.
...
void SpecificWorker::cleanup_owned_nodes()
{
    ...
    // and deleted each node by id — but [Owns].nodes already carries a WILDCARD ("hood_*") that
...
// SHARED (common/owned_nodes): the deterministic "aff_hood*" name test — orphan-safe, it still
