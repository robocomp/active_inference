    ...
    : GenericWorker(configLoader, tprx)
{
    ...
    // default silently.
    int period = 0;
    rc::cfg::Reader(configLoader, "human_concept").req("Period.Compute", period,
...
void SpecificWorker::initialize()
{
    ...
    // ARMS the unread sweep; check_registry_complete.sh human_concept is the grep that keeps the claim
    // honest. Re-run it whenever a config read is added.
    rc::cfg::exempt_generated_prefixes();
    rc::cfg::registry().declare_complete("human_concept");
    rc::cfg::Reader(configLoader, "human_concept").publish("etc/config_effective.csv");
}
