...
```
  ...
  Install the library Pinocchio as:
  
  sudo mkdir -p /etc/apt/keyrings

  curl -fsSL http://robotpkg.openrobots.org/packages/debian/robotpkg.asc | sudo tee /etc/apt/keyrings/robotpkg.asc > /dev/null

  echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/robotpkg.asc] http://robotpkg.openrobots.org/packages/debian/pub noble robotpkg" | sudo tee
...
Like any other component, kinova_controller requires a configuration file to start. In etc/config or etc/config.toml, you can find an example of the configuration file.
...
bin/kinova_controller etc/yourConfig # Execute the component
```
-----
-----
# Developer Notes
This section explains how to work with the generated code of kinova_controller, including what can be modified and how to use key features.
## Editable Files
