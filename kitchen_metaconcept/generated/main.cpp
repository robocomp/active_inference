...
/** \mainpage RoboComp::kitchen_metaconcept
 *
 * \section intro_sec Introduction
 *
 * The kitchen_metaconcept component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/kitchen_metaconcept --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
 ...
 */
#include <signal.h>
#include <clocale>   // std::setlocale — LC_NUMERIC pin, see run()
...
#define PROGRAM_NAME    "kitchen_metaconcept"
#define SERVER_FULL_NAME   "RoboComp kitchen_metaconcept::kitchen_metaconcept"



class kitchen_metaconcept : public Ice::Application
{
public:
	kitchen_metaconcept (QString configFile, QString prfx, bool startup_check) { 
...
Ice::InitializationData kitchen_metaconcept::getInitializationDataIce(){
...
void kitchen_metaconcept::initialize()
{
...
int kitchen_metaconcept::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	// The Q*Application constructor calls setlocale(LC_ALL, "") — under LANG=es_ES.UTF-8 that makes the C
	// library read a COMMA as the decimal separator, while std::ofstream keeps writing POINTS through the
	// C++ global locale. strtof/atof then silently truncate our own files ("0.626452" -> 0). Pin LC_NUMERIC
	// back to "C" so the two agree. This is what robocompdsl now emits for newly generated components
	// (CLAUDE.md); this file was cloned from a pre-fix template, so the line is added by hand.
...
int main(int argc, char* argv[])
{
	...
	kitchen_metaconcept app(configFile, prefix, startup_check_flag);
