...
// regeneration silently reverts the change below and re-arms a bug that damages the SHARED graph.
//
// The patch: ~GenericWorker() had a body that deletes every "grid"-typed node from every graph.
// See the comment on the destructor for why that is destructive for this particular agent.
// ##################################################################################################
/**
...
GenericWorker::~GenericWorker()
{
    ...
    //         for (auto grid : graphPtr->get_nodes_by_type("grid"))
    //             graphPtr->delete_node(grid);
    //
    // i.e. on shutdown it deletes EVERY node of type "grid" from EVERY graph it holds. One of this
    // agent's graphs is the LIVE shared working graph, and residual_concept publishes a node NAMED
    // "residual" whose TYPE is still grid_node_type (residual_concept/src/specificworker.cpp:1683).
    // So the stock destructor would silently delete another agent's live node out of the shared
    // graph every time ltsm_agent is stopped -- a node this agent neither created nor owns.
    //
    // ltsm_agent creates no "grid" nodes and owns nothing to reap here, so the body is empty.
    // Deleting only what you created is the fleet's ownership rule; see CONCEPT_AGENT_LIFECYCLE.md
    // and common/owned_nodes/owned_nodes.h.
    //
    // ⚠ generated/ is NOT in robocompdsl's avoid_overwrite list (templateCPP/templatecpp.py:20-23),
    // so RE-RUNNING THE GENERATOR RESTORES THE DESTRUCTIVE VERSION. Re-apply this patch if you do.
    // ############################################################################################
}
