...
EpistemicProposal EpistemicPlanner::compute(const RefrigeratorBelief& belief, float lat_rate, float sigma_base) const
{
    ...
    // ★PLACEHOLDER target precision Σ* — the posterior covariance at which the refrigerator is "adequately
    ...
    // tuned Σ bound. A mode-ambiguous refrigerator (marginal σyaw≈45°) stays inadequate on the yaw DOF until the
