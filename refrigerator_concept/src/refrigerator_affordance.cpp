/*
 * refrigerator_affordance.cpp  —  affordance-node lifecycle + controller-protocol state machine.
 *
 * Creates/refreshes/removes the per-refrigerator affordance DSR node, writes the epistemic target + the shared
...
namespace rc {
void RefrigeratorAffordance::init(std::shared_ptr<DSR::DSRGraph> G,
                            uint64_t    refrigerator_node_id,
                            std::string refrigerator_node_name)
{
    G_               = std::move(G);
    refrigerator_node_id_   = refrigerator_node_id;
    refrigerator_node_name_ = std::move(refrigerator_node_name);
}

void RefrigeratorAffordance::update(const EpistemicProposal& prop)
{
    if (not G_) return;
...
void RefrigeratorAffordance::remove()
{
    ...
    std::print("[affordance] removed node for '{}' (state={})\n",
               refrigerator_node_name_, state_name(state_));
    reset();   // returns to State::idle so a new cycle can start if the model degrades later
}

void RefrigeratorAffordance::on_node_modified(uint64_t id)
{
...
void RefrigeratorAffordance::on_node_deleted(uint64_t id)
{
    if (not node_created_ or id != affordance_node_id_) return;
...
std::string_view RefrigeratorAffordance::state_name(State s)
{
    switch (s)
    {
...
// all four refrigerator faces in object frame + their adequacy-bounded gains + the stand-off band + framing +
...
void RefrigeratorAffordance::create_node(const EpistemicProposal& prop)
{
    const std::string aff_name = "aff_" + refrigerator_node_name_;

    DSR::Node aff_node = DSR::Node::create<affordance_node_type>(aff_name);
    G_->add_or_modify_attrib_local<level_att>                   (aff_node, 4);
    G_->add_or_modify_attrib_local<parent_att>                  (aff_node, refrigerator_node_id_);
    ...
    // inserted and validate the round-trip + value sanity — faces↔gains parity (4 for a refrigerator), finite
    ...
    if (auto rb = G_->get_node(affordance_node_id_); rb.has_value())
    {
        ...
        if (ok)
            ...
            std::print("[affordance-selfcheck] '{}' viewpoint MISMATCH: object_relative={} faces={} gains={} "
                       "σ*={} standoff=[{:.2f},{:.2f}]\n",
                       refrigerator_node_name_, vc.object_relative, vc.faces.size(), vc.face_gains.size(),
                       vc.sigma_star.size(), vc.standoff_min_m, vc.standoff_max_m);
    }
}

void RefrigeratorAffordance::update_node(const EpistemicProposal& prop)
{
    auto node_opt = G_->get_node(affordance_node_id_);
    if (not node_opt.has_value())
    {
...
void RefrigeratorAffordance::refresh_edge()
{
    auto edge = DSR::Edge::create<has_intention_edge_type>(refrigerator_node_id_, affordance_node_id_);
    G_->insert_or_assign_edge(edge);
}

void RefrigeratorAffordance::reset()
{
