/*
 * refrigerator_fitter.cpp — the active-inference fit core for refrigerator_concept (see refrigerator_fitter.h).
 *
...
RefrigeratorFitter::RefrigeratorFitter(std::shared_ptr<DSR::DSRGraph> graph,
                         DSR::InnerEigenAPI* inner_eigen,
                         RefrigeratorConfig& cfg,
                         MaskIngestor* mask_ingestor,
                         RefrigeratorSceneGraph* scene_graph)
    : G_(graph), inner_eigen_(inner_eigen), cfg_(cfg),
      mask_ingestor_(mask_ingestor), scene_graph_(scene_graph),
      projection_(std::make_unique<RefrigeratorProjection>(graph, inner_eigen, mask_ingestor))
{
    RefrigeratorBeliefState::use_quotient = cfg.quotient_chart;   // C2v symmetry-quotient optimisation chart (global mode)
...
// extrinsic the fit uses (RefrigeratorProjection), pinned to the CURRENT pose (ts=0). Call once per compute cycle.
void RefrigeratorFitter::update_ego_motion()
{
...
float RefrigeratorFitter::motion_magnitude(const RefrigeratorInstance& inst) const
{
...
float RefrigeratorFitter::periphery_penalty(const RefrigeratorInstance& inst) const
{
...
void RefrigeratorFitter::resolve_front_from_rgb(RefrigeratorInstance& inst, float mode_evidence_weight)
{
    ...
    RefrigeratorState box = inst.model.state();
    box.cx = bs.cx; box.cy = bs.cy; box.refrigerator_height = bs.H; box.w = bs.w; box.h = bs.h; box.yaw = bs.yaw;
...
void RefrigeratorFitter::set_chain_cov_source(DSR::InnerGaussianAPI* gaussian, std::string source_frame)
{
    gaussian_          = gaussian;
...
void RefrigeratorFitter::set_object_observation(bool enabled, std::string robot_frame)
{
...
// re-anchors to the last pose (the residual is ~0 at the current estimate). So we take the refrigerator's
...
void RefrigeratorFitter::compute_object_observation(RefrigeratorInstance& inst)
{
    ...
    // This frame's ZED slice assigned to this refrigerator (the pinhole camera cloud; skip ricoh depth_var>0).
    ...
    // box_yaw in body = belief yaw (room) + yaw(body←room). See REFRIGERATOR_TRIANGULATION.md.
    {
...
// Compute the localization/chain covariance term (J·Σ_chain·Jᵀ) at the refrigerator centre; store it on the instance.
//
...
// capture stamp. The refrigerator is fit in room but its position stays conditional on the robot pose
// (camera→robot→room), so this per-frame SHARED localization error feeds the belief common-mode.
void RefrigeratorFitter::compute_chain_cov(RefrigeratorInstance& inst)
{
    inst.chain_cov_xx = 0.0f;
    inst.chain_cov_yy = 0.0f;
    if (not chain_cov_enabled_ or not gaussian_ or not inner_eigen_)
        return;
    // Localization/chain term J·Σ_chain·Jᵀ at the refrigerator centre: transform it to the measurement frame,
    // then back to room with ZERO input cov — InnerGaussianAPI returns exactly the chain contribution
    // (Σ_chain from each RT edge's rt_covariance), pinned to the mask capture stamp. The refrigerator is fit in
    // room but its position is still conditional on the robot pose (camera→robot→room), so this applies.
    const auto& s = inst.model.state();
    const Mat::Vector3d centre(s.cx, s.cy, 0.0);   // refrigerator node origin = base on the floor (z=0)
...
// Create the instance for a "refrigerator_*" node if absent; return true only the first time it is created.
//
// Warm-starts from the node's size attribs and the room→refrigerator RT edge, then OVERRIDES with the tracker
...
bool RefrigeratorFitter::ensure_instance(const DSR::Node& node, std::uint64_t room_id)
{
    room_node_id_ = room_id;
    if (instances_.count(node.id()))
        return false;

    RefrigeratorState init_state;
    init_state.cx  = 0.0f;
    init_state.cy  = 0.0f;
    init_state.yaw = 0.0f;
    ...
    if (auto v = G_->get_attrib_by_name<height_m_att>(node); v.has_value()) init_state.refrigerator_height = v.value();

    // Read RT pose from room→refrigerator edge
    if (room_node_id_ != 0)
    {
    ...
    init_state.leg_length = std::max(0.05f, init_state.refrigerator_height - RefrigeratorModel::TOP_THICKNESS);
    ...
    // optimizer; replace any non-finite field with a safe default before it reaches the model.
    {
        const auto fix = [&](float& v, float fallback, const char* name)
        {
            if (!std::isfinite(v))
            {
                std::print("refrigerator_concept: WARNING non-finite {} for '{}' → reset to {:.3f}\n", name, node.name(), fallback);
                v = fallback;
            }
        };
        ...
        fix(init_state.refrigerator_height, 1.70f, "refrigerator_height");
        fix(init_state.leg_inset, RefrigeratorModel::LEG_RADIUS, "leg_inset");  // FROZEN at outer edge
        init_state.leg_length = std::max(0.05f, init_state.refrigerator_height - RefrigeratorModel::TOP_THICKNESS);
    }

    RefrigeratorInstance inst;
...
// The caller (process_refrigerator_node) invokes this once per assigned slice and runs a belief update for each —
...
RefrigeratorFitter::RefrigeratorObservation RefrigeratorFitter::observe_slice(RefrigeratorInstance& inst, int slice_index)
{
    RefrigeratorObservation observation;
    ...
    // A slice assigned to this refrigerator ⇒ detection is alive; latch its per-slice R inputs.
...
RefrigeratorFitter::RefrigeratorObservation RefrigeratorFitter::observe(RefrigeratorInstance& inst, const DSR::Node& node)
{
    ...
    (void)inst; (void)node;
    return RefrigeratorObservation{};
}

// True on the config-driven log period (one in refrigerator_log_period_frames cycles).
bool RefrigeratorFitter::should_log(const RefrigeratorInstance& inst) const
{
    const int period = std::max(1, cfg_.refrigerator_log_period_frames);
...
WallRef RefrigeratorFitter::nearest_wall(const Eigen::Vector2f& q) const
{
    WallRef w;
...
float RefrigeratorFitter::run_inference(RefrigeratorInstance& inst, const RefrigeratorObservation& observation)
{
    ...
    if (not inst.ai2_initialized)
    {
        ...
        p.top_thickness   = RefrigeratorModel::TOP_THICKNESS;
        p.leg_radius      = RefrigeratorModel::LEG_RADIUS;
    ...
    // information-filter axiom), so a dead mask/ZED feed read downstream as a confident-but-stale refrigerator.
    ...
    // unobservable: a 7 m view should confirm existence but never rotate a converged refrigerator. The
    ...
    // Pose-chain covariance at the refrigerator centre (cx,cy) — the per-frame SHARED localization error. Fed
    ...
    // not measure. A near-horizontal camera→refrigeratortop ray foreshortens the top face, so its 2D footprint (hence
    ...
    if (const auto rTz = projection_->room_T_zed_matrix(inst.last_mask_timestamp_ms); rTz)
    {
        ...
        const Eigen::Vector3d ray(s0b.cx - cam.x(), s0b.cy - cam.y(), s0b.H - cam.z());   // camera→refrigeratortop centre
    ...
    else
    {
        ...
        // tilt STATE — computed from the TRUE extrinsic, NOT the belief's drifting refrigerator-centre, so a persistent
        ...
        // Obliquity yaw cap: at an edge-on (grazing) view the refrigeratortop cloud is ~1-D along the near edge, so yaw
        ...
        // grazing frame confirms the refrigerator but cannot rotate it — the same continuous-covariance form as the range
        ...
        // deliberately kept. Its intended derived replacement, RefrigeratorBelief::tilt_yaw_common_mode() (a2′), is WIP and
        ...
        // tilt_yaw_common_mode is called here and validated. See refrigerator_belief.cpp::tilt_yaw_common_mode.
        ...
        // in). Mirror the per-point obliquity cap onto moment_extra_var so an edge-on frame confirms the refrigerator but
        ...
        // or no fresh sweep. The shared factor (accumulate_lidar_rays<6> in RefrigeratorBelief::accumulate_extra)
        ...
        // a grazing view's yaw-carrying points get huge R → the per-point GN cannot rotate a converged refrigerator,
        ...
        // static refrigerator cannot physically move max_step_m in one frame, so such a step is definitionally spurious.
        const RefrigeratorBelief pre_belief = inst.ai2_belief;   // value copy (state + Σ + prior + flip_evidence)
        const auto&       ps         = pre_belief.state();
        energy = inst.ai2_belief.update(frame);
        const auto& ns = inst.ai2_belief.state();
        // Full-state jump: centre (cx,cy,H) AND EXTENT (w,h). A static refrigerator can't grow/shrink its extent by
        ...
        else
        {
            ...
            // sustained rise = the refrigerator moved shows as surprise before the baseline accepts it); surprise = the
        ...
        // until an orbit resolves it. See REFRIGERATOR.md.
        ...
        if (not rc::RefrigeratorBeliefState::use_quotient
    ...
    // Write the belief back into the legacy RefrigeratorState so all downstream publish/viewer/RT code is
    // unchanged. Legs are derived: leg_length = H − TOP_THICKNESS, inset frozen at the outer edge.
    const auto& bs = inst.ai2_belief.state();
    RefrigeratorState ms = inst.model.state();
    ms.cx = bs.cx; ms.cy = bs.cy; ms.refrigerator_height = bs.H; ms.w = bs.w; ms.h = bs.h; ms.yaw = bs.yaw;
    ms.leg_length = std::max(0.05f, bs.H - RefrigeratorModel::TOP_THICKNESS);
    ms.leg_inset  = RefrigeratorModel::LEG_RADIUS;
    ...
    inst.dbg_gate_fresh = true;   // computed from THIS frame's mask — see refrigerator_instance.h
...
// STRIPPED for refrigerator_concept: a refrigerator has a single fixed CLASS (a solid box) — there is no
// round/square shape hypothesis to select. The subtype is the constant class discriminator "refrigerator"
...
void RefrigeratorFitter::evaluate_shape(RefrigeratorInstance& inst)
{
    inst.subtype = "refrigerator";
}

// Append one AI2 belief row (state + Σ-diag std + mask R/bias/trunc + mode evidence + LiDAR diag) to the CSV.
void RefrigeratorFitter::log_ai2_csv(const RefrigeratorInstance& inst, int npts, float R, bool gated, float energy)
{
    ...
    if (not ai2_csv_.is_open())
    {
        ...
        ai2_csv_ << "cycle,node,pkt_fid,pkt_ts,npts,gated,energy,fe_baseline,fe_surprise,R,motion_var,depth_var,motion_dotd,trunc_frac,range,"
                 ...
                 << "nbv_standoff,nbv_tx,nbv_ty,nbv_pdetect,nbv_vfov,nbv_gain_raw,nbv_gain_pub,ricoh_det,ricoh_attn\n";
    }
...
// Build the RefrigeratorModel params from config (the SDF split band for candidate/residual classification).
RefrigeratorModelParams RefrigeratorFitter::make_model_params() const
{
    RefrigeratorModelParams p;
...
bool RefrigeratorFitter::self_test()
{
    bool all = true;
    RefrigeratorConfig cfg;   // defaults: AI2MotionConfirmOnly=true, still-levels, ang-lever=2.0, center-radius=0.35
    RefrigeratorFitter f(nullptr, nullptr, cfg, nullptr, nullptr);
    ...
    std::print("RefrigeratorFitter::self_test [motion_magnitude]   {}  lin={:.3f} ang={:.3f} dotd={:.3f}\n",
    ...
    RefrigeratorInstance ci; ci.last_motion_dotd = 0.0f;
    ...
    std::print("RefrigeratorFitter::self_test [confirm_only]       {}  move_off={} still={} move_centre={}\n",
    ...
    RefrigeratorBeliefParams P;
    const RefrigeratorBeliefState s0{0.0f, 0.0f, 1.70f, 0.60f, 0.60f, 0.0f};
    std::mt19937 rng(777); std::normal_distribution<float> nz(0.0f, 0.01f);
    RefrigeratorFrame fr;                                          // a box cloud offset from the seed by (0.15,-0.10)
