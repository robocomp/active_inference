/*
 * refrigerator_fitter.h — the active-inference fit core of refrigerator_concept (mirrors bottle_concept/bottle_fitter.h).
 *
...
#include "refrigerator_instance.h"      // rc::RefrigeratorInstance, RefrigeratorState
#include "refrigerator_model.h"         // RefrigeratorModel / RefrigeratorModelParams
#include "refrigerator_projection.h"    // rc::RefrigeratorProjection, rc::SilhouetteExistence
#include "refrigerator_lidar_range_channel.h"   // rc::RefrigeratorLidarRangeChannel
...
{
public:
    struct RefrigeratorObservation
    {
    ...
    RefrigeratorFitter(std::shared_ptr<DSR::DSRGraph> graph,
                DSR::InnerEigenAPI* inner_eigen,
                RefrigeratorConfig& cfg,
                MaskIngestor* mask_ingestor,
                RefrigeratorSceneGraph* scene_graph);

    // Create the instance for a "refrigerator_*" node if absent (from prior/RT). Returns true the
    ...
    RefrigeratorObservation observe_slice(RefrigeratorInstance& inst, int slice_index);
    ...
    RefrigeratorObservation observe(RefrigeratorInstance& inst, const DSR::Node& node);
    // One recursive full-covariance belief update (RefrigeratorBelief) on this frame's mask points, with the
    ...
    float run_inference(RefrigeratorInstance& inst, const RefrigeratorObservation& observation);
    ...
    // update"). Ported 1:1 from chair_concept. Uses the SAME room_T_zed extrinsic (RefrigeratorProjection) the fit uses.
    ...
    float motion_magnitude(const RefrigeratorInstance& inst) const;
    // Off-axis penalty ∈ [0,1]: 0 on the optical axis (a centred mask has no peripheral smear), → 1 at ai2_periph_ref.
    float periphery_penalty(const RefrigeratorInstance& inst) const;
    ...
    std::unordered_map<std::uint64_t, RefrigeratorInstance>& instances() { return instances_; }
    ...
    // use the refrigerator as an SE(2) pose landmark. robot_frame = the localizer's base node ("body"). OFF unless enabled.
    ...
    // RefrigeratorExistence::update_and_remove for non-observed instances only, so it never double-logs a fit row.
    void log_existence_cycle(const RefrigeratorInstance& inst)
    ...
    // fresh LiDAR. Set/cleared from the compute() main thread by SpecificWorker (fed by RefrigeratorLidarIngestor).
...
private:
    // Compute the localization/chain covariance term (J·Σ_chain·Jᵀ) at the refrigerator centre by transforming
    ...
    RefrigeratorModelParams  make_model_params() const;

    // Append one AI2 belief row (state + Σ diag std + mask R/bias/trunc + gate flag) to cfg_.ai2_csv_path.
    void log_ai2_csv(const RefrigeratorInstance& inst, int point_count, float R, bool gated, float energy);
    ...
    void compute_object_observation(RefrigeratorInstance& inst);      // fill inst.obs_robot (gated)
    RefrigeratorConfig&                   cfg_;
    MaskIngestor*                  mask_ingestor_ = nullptr;
    RefrigeratorSceneGraph*               scene_graph_   = nullptr;
    std::unique_ptr<RefrigeratorProjection> projection_;   // camera-projection unit (owns the ZED CameraAPI)
    ...
    std::unordered_map<std::uint64_t, RefrigeratorInstance> instances_;
    ...
    RefrigeratorLidarRangeChannel         lidar_channel_{cfg_};
    ...
    void resolve_front_from_rgb(RefrigeratorInstance& inst, float mode_evidence_weight);
};
