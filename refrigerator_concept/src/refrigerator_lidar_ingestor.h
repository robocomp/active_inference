/*
 * refrigerator_lidar_ingestor.h  —  lidar3D media-plane consumer for the YOLO-independent range factor
 *
 ...
 * LiDAR first-hit factor needs (see common/ai_belief/lidar_ray_factor.h). Because RefrigeratorBelief exposes the
...
class RefrigeratorLidarIngestor
{
public:
    RefrigeratorLidarIngestor(std::shared_ptr<DSR::DSRGraph> graph, DSR::InnerEigenAPI* inner_eigen,
                       const RefrigeratorConfig& cfg);
    ~RefrigeratorLidarIngestor();
