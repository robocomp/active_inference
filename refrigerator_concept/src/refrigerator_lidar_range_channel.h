/*
 * refrigerator_lidar_range_channel.h — YOLO-independent LiDAR range channel for refrigerator_concept (extracted from RefrigeratorFitter).
 *
 * Owns the staged per-cycle sweep (room frame + sensor origin) and selects the returns landing on ONE refrigerator,
 * staging them on the frame's LiDAR channel for the first-hit range factor / free-space VACATE term:
 * set_sweep (stage this cycle's sweep + origin, from RefrigeratorFitter fed by RefrigeratorLidarIngestor), clear (drop the
 * staged sweep, called each cycle before set_sweep so a stale sweep never leaks), and feed (select this
 * refrigerator's returns and populate frame.lidar). Holds the RefrigeratorConfig by reference. Plain class (no Q_OBJECT).
 */
...
#include "refrigerator_belief.h"     // rc::RefrigeratorFrame

namespace rc {

class RefrigeratorLidarRangeChannel
{
public:
    explicit RefrigeratorLidarRangeChannel(const RefrigeratorConfig& cfg) : cfg_(cfg) {}
    ...
    // Select this cycle's LiDAR returns landing on THIS refrigerator and stage them on the frame's range channel(s).
    void feed(RefrigeratorInstance& inst, RefrigeratorFrame& frame) const;

private:
    // Select returns from `sweep` (sensor at `origin`) that land on this refrigerator into `out`, with the two
    ...
    int select_into(const RefrigeratorInstance& inst, const Eigen::Vector3f& centroid,
