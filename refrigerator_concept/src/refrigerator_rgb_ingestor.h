/*
 * refrigerator_rgb_ingestor.h  —  ZED RGB media-plane consumer for appearance-based FRONT (door) detection
 *
 * Sibling of refrigerator_lidar_ingestor.h (the sanctioned in-agent media-plane pattern). Brings up the shared
 ...
 * pixels RefrigeratorProjection::detect_front scores for door-ness. The fitted 3D box (not YOLO's 2D mask) is what
 * gets projected, so this keeps working when the robot is too close for YOLO to detect the fridge.
 *
 * Crash-safety (mirrors RefrigeratorLidarIngestor / room_concept::CameraVisualizer):
...
class RefrigeratorRgbIngestor
{
public:
    RefrigeratorRgbIngestor(std::shared_ptr<DSR::DSRGraph> graph, const RefrigeratorConfig& cfg);
    ~RefrigeratorRgbIngestor();
...
private:
    ...
    const RefrigeratorConfig*                   cfg_ = nullptr;
