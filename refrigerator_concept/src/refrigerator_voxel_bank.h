/*
 * refrigerator_voxel_bank.h — the refrigerator-owned voxel memory (extracted from RefrigeratorFitter).
 *
 * Header-only free functions in rc::voxel_bank that maintain each instance's persistent point bank: key()
 * (FNV-1a quantised voxel key), owned_by_refrigerator() (XY-radius + height ownership gate), and ingest() (insert
 * this cycle's candidate/residual points, cap-bounded, ownership-gated, de-duplicated by key). Depends only
 * on rc::RefrigeratorInstance and rc::RefrigeratorConfig, so it never pulls in RefrigeratorFitter's nested observation type.
 */
...
//
// Two THRESHOLDS, both physical, keep a neighbouring refrigerator's or the floor's points out of the bank:
// an XY radius (refrigerator half-diagonal + config margin, floored at 1 m so a tiny model still owns nearby
// returns) and a vertical band [z_min, refrigerator_height + config margin].
inline bool owned_by_refrigerator(const RefrigeratorInstance& inst, const Eigen::Vector3f& point, const RefrigeratorConfig& cfg)
{
    const auto& s = inst.model.state();

    // XY ownership gate: refrigerator-centered radius with a configurable margin.
    ...
    const float z_max = s.refrigerator_height + cfg.voxel_select_height_margin_m;
...
inline void ingest(RefrigeratorInstance& inst,
    ...
    auto ingest_pts = [&](const std::vector<Eigen::Vector3f>& src)
    {
        for (const auto& p : src)
        {
            if (inst.voxel_bank_pts.size() >= max_points)
                break;
            if (not owned_by_refrigerator(inst, p, cfg))
            {
                ++rejected_foreign;
                continue;
            }
