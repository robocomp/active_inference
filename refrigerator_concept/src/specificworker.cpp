      ...
      startup_check_flag(startup_check)
{
    ...
    // default silently.
    int period = 0;
    rc::cfg::Reader(configLoader, "refrigerator_concept").req("Period.Compute", period,
...
void SpecificWorker::initialize()
{
    ...
    // ARMS the unread sweep; check_registry_complete.sh refrigerator_concept is the grep that keeps the claim
    // honest. Re-run it whenever a config read is added.
    rc::cfg::exempt_generated_prefixes();
    rc::cfg::registry().declare_complete("refrigerator_concept");
    rc::cfg::Reader(configLoader, "refrigerator_concept").publish("etc/config_effective.csv");
}
...
void SpecificWorker::log_birth_surprise()
{
    ...
    // Believed refrigerator footprints (room frame) — a region under one is already explained, NOT a birth. This is
    // the one genuinely per-object step: a refrigerator's two footprint extents. Everything after it — both CSVs,
    ...
    birth_surprise_log_.write(residual_field_, footprints, last_refrigerator_dets_xy_,
