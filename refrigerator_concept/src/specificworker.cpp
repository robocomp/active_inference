...
void SpecificWorker::initialize()
{
    ...
    lidar_ingestor_ = std::make_unique<rc::RefrigeratorLidarIngestor>(G, inner_eigen_.get(), cfg_);
...
void SpecificWorker::log_birth_surprise()
{
    ...
    for (const auto& c : cands) if (not c.covered_by_refrigerator) ++n_birth;
    ...
    if (birth_surprise_csv_.is_open())
    {
        ...
        for (const auto& c : cands)
            birth_surprise_csv_ << cyc << ',' << r++ << ',' << c.cx << ',' << c.cy << ',' << c.cells << ','
                                ...
                                << ',' << (c.covered_by_refrigerator ? 1 : 0) << ',' << refrigerators.size() << ','
    ...
    if (n_birth > 0 and (ev_g_.births > 0 or (birth_surprise_log_ctr_++ % 20) == 0))
    {
        ...
        for (const auto& c : cands) if (not c.covered_by_refrigerator) { top = &c; break; }   // cands sorted by mass
        if (top)
