...
void SpecificWorker::refresh_belief_inspector()
{
    ...
    for (const auto& [id, inst] : fitter_->instances())
    {
        ...
        for (int j = 0; j < rc::RefrigeratorBelief::N; ++j)
            c.dofs.push_back({rc::kRefrigeratorDofs[j].name, rc::kRefrigeratorDofs[j].unit, v[j],
                              std::sqrt(std::max(0.0f, S(j, j))), rc::kRefrigeratorDofs[j].sigma_star});
        ...
        constexpr int N = rc::RefrigeratorBelief::N;
        c.cov.resize(N * N);
        for (int i = 0; i < N; ++i)
            for (int j = 0; j < N; ++j)
                c.cov[i * N + j] = S(i, j);
...
// Remove timeseries series for refrigerators no longer present in the graph, so a removed refrigerator's lines don't persist
// (and a refrigerator reborn under the same name gets fresh series via the idempotent add_series in the feed). Runs
...
void SpecificWorker::prune_dead_series()
{
    ...
    for (auto it = ts_known_refrigerators_.begin(); it != ts_known_refrigerators_.end();)
    {
        ...
        it = ts_known_refrigerators_.erase(it);
    }
    for (const auto& [id, inst] : fitter_->instances()) ts_known_refrigerators_.insert(inst.node_name);
}
...
// the widget pointers stay null and the compute feed (publish_refrigerator_diagnostics / refresh_evidence_monitor /
...
void SpecificWorker::build_dashboard()
{
    ...
    {
        custom_widget_ = new Custom_widget("Refrigerator — Free Energy, Surprise, Belief Uncertainty & Residuals");
        ...
        // (~2–8), so it needs the full panel height to be readable. Spikes when a refrigerator moves, decays as the fit
        ...
        // Per-instance series are registered idempotently by the diagnostics feed (publish_refrigerator_diagnostics)
    ...
    evidence_monitor_ = new rc::EvidenceMonitor(QStringLiteral("refrigerator_concept — evidence monitor"));

    // ── Combined window: evidence monitor (top) over the belief plots (bottom) in a resizable splitter ──
    dashboard_window_ = new QWidget;
    dashboard_window_->setWindowTitle(QStringLiteral("refrigerator_concept — dashboard"));
