/*
 * specificworker_presence.cpp — presence protocol + owned-node cleanup for refrigerator_concept.
 *
 ...
 * nodes — "refrigerator_*" instances and their child "affordance" nodes — both as a one-time startup stale-sweep
...
// keyed on refrigerator's PRIMARY input (the voxelizer `masks` node) instead of a LiDAR media plane.
...
void SpecificWorker::remove_owned_refrigerator_nodes()
{
    if (not G)
        return;

    // Refrigerator instances are DSR `refrigerator` nodes named "refrigerator_*". Deleting the node also drops its
    // room→refrigerator RT edge, so no separate edge cleanup is needed. Startup stale-sweep (mirrors bottle).
    ...
    for (const auto id : to_delete)
        if (G->delete_node(id))
            std::print("refrigerator_concept: removed refrigerator node id={}\n", id);
}
...
void SpecificWorker::cleanup_owned_nodes()
{
    ...
    // Sweep every affordance node parented to a refrigerator (robust to renames / orphans not tracked in
...
// Remove every "affordance" node whose parent object is a refrigerator — i.e. this agent's affordances,
...
void SpecificWorker::remove_stale_affordance_nodes()
{
    ...
    {
        // Ours if EITHER (a) the name matches our deterministic "aff_refrigerator*" scheme — orphan-safe,
        // catches affordances whose parent refrigerator node was already deleted (retirement / crashed run) —
        // OR (b) it still hangs from a live refrigerator (backstop for a DSR collision-renamed node).
        bool ours = aff.name().starts_with("aff_refrigerator");
        const char* why = "name";
        if (not ours)
            if (const auto pid = G->get_attrib_by_name<parent_att>(aff); pid.has_value())
                if (const auto parent = G->get_node(pid.value());
                    ...
                    and parent->name().starts_with("refrigerator"))
                {
                    ours = true;
                    why = "parent refrigerator";
                }
        if (ours)
        {
            qInfo() << "[refrigerator_concept] removing affordance node"
