...
void GenericWorker::initialize(){
    for (const auto& [name, Graph] : Graphs) {
        ...
        // Re-run the twopi graph layout once, shortly after the agent node first appears, so the
        // node-link view is ordered as soon as this agent (and its created nodes) join the graph.
