/*
 * residual_lidar_ingestor.h  —  lidar3D media-plane consumer (the residual concept's PRIMARY input)
 *
 * Brings up the shared zero-copy LidarFrame subscriber (same descriptor-driven factory every agent uses)
 * and, once per compute() cycle, stages the newest sweep transformed into the ROOM frame plus the sensor
 * origin. For the residual concept this sweep is the whole perception input — it is fed to the
 * ResidualClusterer (residual filter → 3D DBSCAN), NOT to a YOLO/silhouette factor.
 *
...
class ResidualLidarIngestor
{
public:
    ResidualLidarIngestor(std::shared_ptr<DSR::DSRGraph> graph, DSR::InnerEigenAPI* inner_eigen,
                          const ResidualConfig& cfg);
    ~ResidualLidarIngestor();
