...
#include "../../common/rt_twist/rt_twist.h"             // rc::rt::robot_body_twist — the producer's own ego-motion channel
...
void SpecificWorker::compute()
{
    ...
    if (const auto robots = G->get_nodes_by_type("robot"); not robots.empty())
        if (const auto tw = rc::rt::robot_body_twist(*G, robots.front()); tw.has_value())
