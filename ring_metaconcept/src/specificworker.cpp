      ...
      startup_check_flag(startup_check)
{
    ...
    // default silently.
    int period = 0;
    rc::cfg::Reader(configLoader, "ring_metaconcept").req("Period.Compute", period,
...
void SpecificWorker::initialize()
{
    ...
    // ARMS the unread sweep; check_registry_complete.sh ring_metaconcept is the grep that keeps the claim
    // honest. Re-run it whenever a config read is added.
    rc::cfg::exempt_generated_prefixes();
    rc::cfg::registry().declare_complete("ring_metaconcept");
    rc::cfg::Reader(configLoader, "ring_metaconcept").publish("etc/config_effective.csv");
}
