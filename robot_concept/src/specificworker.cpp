...
void SpecificWorker::initialize()
{
	...
	// ── This robot's own camera boresights, written into the SHARED extrinsic ─
	// ★ Applied HERE and not in each consumer. The body->camera RT edge is the authoritative
	//   description every agent reads; correcting it once means room_concept, retina and anything
	//   else all agree, instead of each keeping a private offset that can drift apart.
	// ★ IDEMPOTENT, and that matters because the graph is persistent: the value is SET, never added,
	//   so a restart re-applies the same angle rather than compounding it. Both robots' JSONs carry a
	//   nominal camera rotation of exactly [0,0,0], so setting the yaw component loses nothing; the
	//   x and y components are preserved regardless.
	// ⚠ Nothing is written when the configured angle already matches the edge — an unmeasured
	//   boresight (Shadow's, 0) must not cost an RT write and a graph signal on every start.
	if (auto rt = G->get_rt_api())
	{
		const std::pair<const char*, float> boresights[] = {
			{"zed",   params.BORESIGHT_YAW_ZED},
			{"ricoh", params.BORESIGHT_YAW_RICOH}};
		for (const auto& [cam, yaw] : boresights)
		{
			auto cn = G->get_node(cam);
			auto bn = G->get_node("body");
			if (not cn.has_value() or not bn.has_value()) continue;   // ALWAYS check (CLAUDE.md)
			const auto e = G->get_edge(bn.value().id(), cn.value().id(), "RT");
			if (not e.has_value()) continue;
			auto rot = G->get_attrib_by_name<rt_rotation_euler_xyz_att>(e.value());
			auto tr  = G->get_attrib_by_name<rt_translation_att>(e.value());
