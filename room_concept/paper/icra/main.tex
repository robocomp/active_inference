...
\caption{\textbf{The estimate is right before it is confident, and says so.} One closed-loop run;
the same room in both panels, at the same scale, long axis horizontal. Pale band: ground truth, each
dimension carrying its measured value and signed error. Discs: per-corner $\sigma$, to scale. The
robot is drawn to scale with its heading, labelled with its bearing relative to the room.
\emph{Left}, parked for \SI{665}{\second}: the geometry is already within \SI{15}{\milli\meter} of
truth, yet every corner declares almost a metre of uncertainty and every edge sits at its prior, so
the layout is withheld---standing still re-observes the same walls from the same viewpoint, and that
evidence saturates. \emph{Right}, the first frame at which the layout becomes publishable, after
turning \SI{319}{\degree} on the spot: the geometry moves by at most \SI{19}{\milli\meter} while
corner $\sigma$ falls by a factor of \num{21}. The estimator does not earn precision by waiting, and
does not claim any.}
\label{fig:teaser}
\end{figure}
...
robot's bearing relative to the room, which is frame-free, and no absolute heading error, which would
describe our own gauge rather than the robot. For the same reason both panels of Fig.~\ref{fig:teaser}
