...
term between \SI{4}{\centi\meter} and \SI{6}{\centi\meter}---the variation a viewpoint-attached error
predicts, since each tour stops at different viewpoints. What is stable is the shape---a factor of six
to nine between the two parts, the bias dominating in \SI{97}{\percent} of pairs in this run---and
that is what the argument uses. A single number here would be a worse claim than the
...
Observations in \S\ref{sec:geom} and \S\ref{sec:calib} are simulated from the annotated polygons; \S\ref{sec:real} validates
