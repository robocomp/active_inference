...
closed-loop run on a real LiDAR model. The pale band is ground truth and each dimension carries its
measured value and signed error: \SI{5}{\milli\meter} on a \SI{6}{\meter} wall is \SI{0.08}{\percent},
too fine to draw at any scale that also fits the room, so the agreement is shown and the discrepancy
is stated. Both panels are drawn with the long axis horizontal; the map frame is gauge-free and the
re-anchor rotates it between the two, but every quantity here---edge lengths, corner $\sigma$, the
comparison with truth---is invariant to that rotation, so what differs between the panels is only the
uncertainty. \emph{Left}, after \SI{665}{\second} parked: the
geometry is already within \SI{15}{\milli\meter} of truth, yet every corner declares almost a metre of
...
\emph{Right}, the first frame at which the layout becomes publishable, after turning
\SI{319}{\degree} on the spot: \SI{24.5}{\second} later, the geometry has moved by at most
\SI{19}{\milli\meter} while corner $\sigma$ has fallen by a factor of \num{21}, under the gate. The
robot is drawn to scale with its heading, and its bearing relative to the room---equivalently to
ground truth, since the truth rectangle lies on the same wall---is printed beneath it: it turns from
\SI{-4}{\degree} to \SI{141}{\degree} while its centre stays inside a \SI{4.4}{\centi\meter} disc and
returns to the same spot in the room to within \SI{1}{\milli\meter}. That bearing is the only
orientation this run can honestly report: the map frame is gauge-free and the log carries no
ground-truth robot pose, so an absolute heading would describe our own gauge rather than the robot.
What the rotation changes is not the room, which does not move, but the precision. The estimator does not earn
...
\textbf{What this measures.} The observations are \emph{simulated from the annotated polygon}: a range sensor is driven through the annotated room and the estimator
...
\textbf{Does the epistemic term earn its place?} The explorer scores viewpoints by expected
...
\emph{The two are indistinguishable.} Paired over the 583 rooms both arms complete, the mean IoU
...
\footnotesize
\num{4344} matched corners from \num{586} rooms. $\sigma$ is the largest-axis standard deviation of
the corner's covariance, so the first three columns are empirical coverage,
$P(\lVert e \rVert_2 \le k\sigma)$ --- \emph{not} a bound: a standard deviation makes no containment
guarantee, and what is tested is whether the declared $\sigma$ predicts the error that occurs.
\emph{publish} is the fraction of corners passing the \SI{6}{\centi\meter} gate and \emph{holds} the
fraction whose claim is borne out. Intervals are \num{2000}-sample bootstraps resampling
\emph{rooms}, not corners: corners of one room share its walls, its trajectory and its annotation, so
a corner-level interval would be far too narrow. The last row is a single global precision calibrated
on this same corpus.
\end{minipage}
\end{table}
...
It did not. Table~\ref{tab:webots} reports the run: \SI{665}{\second} parked, then a
\SI{319}{\degree} turn on the spot.
...
\begin{tabular}{lccc}
\toprule
 & parked, frame 0 & parked, \SI{665}{\second} & after the turn \\
\midrule
long edge   & \SI{5.994}{\meter} & \SI{5.995}{\meter} & \SI{5.994}{\meter} \\
short edge  & \SI{4.015}{\meter} & \SI{4.012}{\meter} & \SI{4.019}{\meter} \\
...
\SI{0.500}{\meter}). A \SI{319}{\degree} turn on the spot, over the next \SI{24.5}{\second}, then
dropped the worst corner $\sigma$ by a factor of \num{21}, to \SI{0.048}{\meter}, while the geometry
moved by millimetres. The third column is the \emph{first} frame the layout is publishable; $\sigma$
keeps falling afterwards, to \SI{0.029}{\meter} a minute later, but the log is sampled at
