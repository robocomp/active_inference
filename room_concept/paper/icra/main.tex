...
never sees a photograph. \textbf{The IoU in Table~\ref{tab:iou} is therefore not comparable with
single-image layout methods evaluated on the same corpus}, which solve a harder problem---recovering
shape from appearance---and should not be scored against these numbers in either direction. What the
...
\eqref{eq:corner}: the median tracks the diagonal and the bound holds. The dotted line is the
publication gate.}
\label{fig:calib}
\end{figure*}
...
room. We report it rather than quoting the \num{0.14} IoU it yields, which measures the task
mismatch and not the estimator.
