...
average is most of an order of magnitude larger than the part that does, and it depends on the
viewpoint rather than on the map---so a constant calibrated in one setting need not remain calibrated
...
\begin{enumerate}
  ...
  part of the error is most of an order of magnitude larger than the averaging part, and it is
  viewpoint-dependent rather than a property of the map (\S\ref{sec:notconstant}).
...
\emph{How much of this is one run.} The magnitude is run-dependent: two tours of this environment,
analysed by the same script, give repeat/scatter ratios of \num{5.6} and \num{8.7}, with the repeating
term between \SI{4}{\centi\meter} and \SI{6}{\centi\meter}. What is stable is the shape---most of an
order of magnitude between the two parts, the bias dominating in \SIrange{92}{97}{\percent} of
pairs---and that is what the argument uses. A single number here would be a worse claim than the
ratio. This is the argument for representing the offset as an inferred state
