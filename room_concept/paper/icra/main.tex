...
\begin{enumerate}
  ...
  simulation, with the size of that bias reproduced on a physical robot (\S\ref{sec:notconstant}).
...
A rigid fit between the detections of consecutive frames recovers the inter-frame motion with no
pose log, so the population can be split by what the robot is \emph{doing}.
Over \num{34} stationary stretches, which fall in \num{23} physical stops because a single moving
frame ends a stretch, and \num{386} corner-stretch pairs, decomposing the innovation for
...
SLAM system---the imperfect map of the kind argued for above---in three
...
\SI{0.0624}{\meter} against \SI{0.0046}{\meter} that scatters, with the bias exceeding the scatter in
all pairs but one. The viewpoint test is weaker than its numbers suggest. The six stretches are
three physical stops, two of them split by a single moving frame, so grouped by stop only one corner
is seen from three or more and the pre-registered test is not supported by this run. Relaxed, post
hoc, to corners seen from two stops, \num{8} corners give a spread \emph{between} stops of
\SI{0.0607}{\meter} against \SI{0.0102}{\meter} \emph{within} one, a factor of \num{5.9}.
Over accepted candidates $\nis$ is \num{0.836}, against \num{0.782} in simulation.

Both verdicts point the way the simulator's do, and the replication is in the bias: \SI{0.0624}{\meter} against
...
stops against \num{23}---and we claim no more than agreement in shape. Its stationary time, about
\SI{36}{\second}, is also below the \SI{60}{\second} our pre-registration required for any
motion-state claim, so none is made on hardware. The protocol called for deliberate
stops at distinct places and orientations; the second session made none, and the first session's
three stretches, which are one stop, gave a between-stretch statistic of \num{1.6}: from a single
place the bias repeats, as a viewpoint-attached error predicts. The third session added two short
stops, and nothing was collected after it. We report the sequence because it
...
\midrule
simulated      & 23 (34) & 386 & \SI{0.0574}{\meter} & \SI{0.0066}{\meter} & \num{8.7}$\times$ & \num{6.0}$\times$ (17) \\
physical robot & 3 (6)  & 46  & \SI{0.0624}{\meter} & \SI{0.0046}{\meter} & \num{13.4}$\times$ & \num{5.9}$\times$ (8)$^\dagger$ \\
\bottomrule
\end{tabular}
...
\begin{minipage}{\columnwidth}
\footnotesize
\emph{stops} are physical stops, with the stationary stretches they contain in brackets; pairs, repeats,
scatters and ratio are per stretch, as pre-registered. \emph{viewpoint} is the spread between
stretches divided by the spread within one, over the corners seen in three or more (count in brackets);
above \num{2} it rules out a fixed map offset. $^\dagger$Post hoc, by stop, over corners seen at two
or more of the three; per stretch it reads \num{9.9}$\times$ (10), but those stretches are not
separate viewpoints.
\end{minipage}
\end{table}

A bias that repeats, follows the viewpoint in simulation and keeps its size on hardware is the argument for
