...
\begin{enumerate}
  ...
  than a property of the map, and the declared pose precision does not cover it
...
\emph{How much of this is one run.} The magnitude is run-dependent: two simulated tours of this
environment, analysed by the same script, give repeat/scatter ratios of \num{5.6} and \num{8.7}---the
variation a viewpoint-attached error predicts, since each tour stops at different viewpoints. What is
stable is the shape, and that is what the argument uses. A single number would be a worse claim than
the range.
...
\num{3}-D LiDAR in an apartment-scale indoor space, over \SI{295}{\meter} in three chained sessions
(Table~\ref{tab:realtour}). Over \num{6} stationary stretches and \num{46} corner-stretch pairs the
repeating part has a median of \SI{0.0624}{\meter} against \SI{0.0046}{\meter} that scatters, a ratio
of \num{13.4}, with the bias exceeding the scatter in \SI{98}{\percent} of pairs; across the \num{10}
corners seen from three or more stretches the spread \emph{between} stretches is \SI{0.0459}{\meter}
against \SI{0.0046}{\meter} \emph{within} one, a factor of \num{9.9}. Both verdicts are the
simulator's. The ratio sits \emph{above} the simulated range rather than inside it, which is the
reason for quoting a range at all: across the four tours it runs from \num{5.6} to \num{13.4} while
the shape does not move.

The real sample is much thinner than the simulated one---\num{46} pairs against \num{386}---and the
run supplied its own reason to say so. With only three stationary stretches the between-stretch
statistic read \num{1.6}, the opposite verdict, because too few genuinely distinct viewpoints had been
visited; deliberately stopping at different places and orientations resolved it. That is a statement
about experimental design rather than about the data: a viewpoint-attached error cannot be detected
without viewpoint diversity, and distance driven is no substitute for it.
...
\toprule
tour & stretches & pairs & repeats & scatters & ratio \\
\midrule
simulated, tour 1 & --- & --- & \SI{0.0403}{\meter} & \SI{0.0063}{\meter} & \num{5.6}$\times$ \\
simulated, tour 2 & 34 & 386 & \SI{0.0574}{\meter} & \SI{0.0066}{\meter} & \num{8.7}$\times$ \\
\textbf{physical robot} & 6 & 46 & \SI{0.0624}{\meter} & \SI{0.0046}{\meter} & \textbf{\num{13.4}$\times$} \\
\bottomrule
\end{tabular}
\end{table} This is the argument for representing the offset as an inferred state
