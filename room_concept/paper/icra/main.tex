...
average is an order of magnitude larger than the part that does, it depends on the viewpoint rather
than on the map, and the precision the system declares for it is constant across motion states in
which the realised error is not---so a constant calibrated in one setting need not remain calibrated
in another, and precision should be inferred rather than tuned.
\end{abstract}
...
\begin{enumerate}
  ...
  part of the error is an order of magnitude larger than the averaging part, it is viewpoint-dependent
  rather than a property of the map, and the precision declared for it is constant across motion
  states in which the realised error is not (\S\ref{sec:notconstant}).
...
In the same simulator, driving a \SI{160}{\meter} tour of a \num{20}-corner apartment, we log for
every corner candidate at the association gate the innovation and each term of its covariance:
\num{148249} candidates over \num{13520} frames, of which the association gate rejects
\SI{17}{\percent}. The tour length is measured from the logged trajectory rather than estimated. The reference layout here is traced from the building's floor plan rather
...
Over \num{12} stationary stretches and \num{139} corner-stretch pairs, decomposing the innovation for
a fixed corner separates two parts cleanly: the part that \emph{repeats} has a median of
\SI{0.0634}{\meter} and the part that \emph{scatters} \SI{0.0064}{\meter}, a ratio of \num{9.9}, with
...
does not: across corners seen in three or more separate stretches, the spread \emph{between}
stretches is \SI{0.0636}{\meter} against \SI{0.0069}{\meter} \emph{within} one---a factor of
\num{9.3}, and as large as the bias itself. It is also not sensor noise, which is the part that
...
The declared precision does not track this. Across the same split, $\sigma_{\text{pred}}$ is flat
---\SI{0.0392}{\meter} at rest against \SI{0.0399}{\meter} in motion---while the realised $\nis$ is
not, at \num{1.97} and \num{1.67}. A precision that is constant across a state in which the error is
not constant is wrong in one of the two states by construction.

\emph{How much of this is one run.} We report a single tour, and the magnitude is run-dependent: three
tours of the same environment give repeat/scatter ratios of \num{5.6}, \num{8.1} and \num{9.9}, with
the repeating term between \SI{4}{\centi\meter} and \SI{10}{\centi\meter}. What is stable across all
three is the shape---an order of magnitude between the two parts, the bias dominating in
\SIrange{92}{97}{\percent} of pairs---and that is what the argument uses. A single number here would
be a worse claim than the ratio. This is the argument for representing the offset as an inferred state
...
\num{0.47} to \num{1.03}. In the \emph{pose} covariance feeding $\sigma_{\text{pred}}$ it is absent again,
