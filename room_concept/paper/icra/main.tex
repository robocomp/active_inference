...
\num{5}$\times$ between the two settings we can measure it in and by \num{2}$\times$ between moving
and standing still, so a constant calibrated in one setting need not remain calibrated in another,
and precision has to be inferred rather than tuned.
\end{abstract}
...
which is what licenses the simulated sensor used in the first level. \S\ref{sec:webots} closes the
loop in simulation with a LiDAR sensor model, which is where uncertainty drives action. No experiment
...
\footnotesize
\num{4344} matched corners from \num{586} rooms: the \num{8} rooms that produced no closed polygon
have no corners to match and are excluded here, leaving \num{586} of the \num{594}. The last row is a
deliberately favourable comparison --- the scalar is fitted \emph{post hoc} on this same corpus so
that its \num{1}$\sigma$ coverage matches ours exactly, information our own estimate never gets.
$\sigma$ is the largest-axis standard deviation of
...
\textbf{The gate is a tolerance, not a tuned parameter.} The \SI{6}{\centi\meter} bound is the
...
solution. These are two settings rather than a sample of environments, so the factor bounds the
