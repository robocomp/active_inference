...
\emph{Right}, after turning \SI{320}{\degree} on the spot---\SI{24}{\second}, and the robot's centre
never leaves a \SI{4.4}{\centi\meter} disc---the same geometry, now with corner $\sigma$ smaller by
a factor of \num{32} and every corner under the publication gate. The turn itself is deliberately not
visible here: the panels draw the \emph{room}, which does not move, and the robot is not drawn; a
figure of the robot would help no more, since \SI{320}{\degree} returns it to nearly its original
bearing. What a rotation changes is not the picture but the precision, which is the quantity the two
panels differ in. The estimator does not earn
