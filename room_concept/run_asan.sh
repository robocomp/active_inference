#!/usr/bin/env bash
# Catch the heap corruption that crashes paintAndFlush, WITHOUT relinking room_concept.
#
...
#
# PREREQ: build the INSTRUMENTED binary so ./bin/room_concept has room_concept's OWN code under ASan
...
# Then run this; start room/robot + bottle_concept in another terminal. Report -> /tmp/room_asan.txt
set -u
cd "$(dirname "$0")"
...
if command -v patchelf >/dev/null && readelf -d ./bin/room_concept 2>/dev/null | grep -qi 'NEEDED.*libasan'; then
    echo "Stripping libasan DT_NEEDED from ./bin/room_concept"
    patchelf --remove-needed libasan.so.8 ./bin/room_concept 2>/dev/null \
        || patchelf --remove-needed "$(basename "$LIBASAN")" ./bin/room_concept
fi
...
echo "Running ./bin/room_concept (ASan-instrumented) under LD_PRELOAD ASan; report -> /tmp/room_asan.txt"
exec ./bin/room_concept etc/config.toml 2>&1 | tee /tmp/room_asan.txt
