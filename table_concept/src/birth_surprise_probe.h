#ifndef TABLE_BIRTH_SURPRISE_PROBE_H
#define TABLE_BIRTH_SURPRISE_PROBE_H
 ...
 * by a believed table is a candidate for a NEW table: a concept condensing out of the residual field (birth =
 ...
 * surprise ramp on a real table's region and confirm it flags births cleanly (and rejects phantoms) BEFORE
 * letting it gate/replace the tracker's frames-counter. Pure math (no DSR) → unit-testable. The removal half is
 * NOT here: the grid subtracts explained cells, so it cannot see a modelled table's absence — removal stays the
 * per-instance existence-surprise job. See [[table-birth-surprise-probe]].
 */
...
// A believed table's footprint (room frame) — used to mark residual regions an existing concept already explains.
...
struct BirthCandidate
{
    ...
    bool  covered_by_table = false;    // centroid inside a believed table footprint (+margin) ⇒ NOT a birth
};

struct BirthProbeParams
{
    float p_threshold    = 0.5f;   // a cell counts as confidently-unexplained-occupied above this P
    float cover_margin_m = 0.30f;  // grow each table footprint by this before the "already explained" test
    int   min_cells      = 4;      // ignore specks (sensor noise); a real table footprint spans many cells
};
...
public:
    ...
    // MASS, flag whether a believed table already covers it, and return them sorted by mass (strongest first).
    // Pure function of its inputs — no DSR, no state.
    static std::vector<BirthCandidate> scan(const GridField& g,
                                            const std::vector<FootprintBox>& tables,
                                            const BirthProbeParams& p = {})
    {
        ...
        for (int s = 0; s < N; ++s)
        {
            ...
            c.covered_by_table = covered(c.cx, c.cy, tables, p.cover_margin_m);
            out.push_back(c);
        }
    ...
    // FUSION signal: a real table detection should land on high residual mass; a flicker/phantom detection on ~0.
    ...
    // Self-test: two blobs (one already under a table, one free); scan must find both, mark exactly one covered,
    ...
    static bool self_test()
    {
        ...
        std::vector<FootprintBox> tables{ {0.85f, 0.85f, 0.9f, 0.9f, 0.0f} };        // covers blob A
        const auto c = scan(g, tables, {});
        bool ok = c.size() == 2;                          // two real regions, speck dropped
        ok = ok and c[0].mass > c[1].mass;                // sorted by mass; B (bigger) first
        ok = ok and not c[0].covered_by_table;            // blob B is free
        ok = ok and c[1].covered_by_table;                // blob A sits under the table
...
private:
    static bool covered(float x, float y, const std::vector<FootprintBox>& tables, float margin)
    {
        for (const auto& t : tables)
        {
            const float c = std::cos(t.yaw), s = std::sin(t.yaw);
            const float dx = x - t.cx, dy = y - t.cy;
            const float lx =  c * dx + s * dy;   // world → table-local (rotate by −yaw)
...
#endif  // TABLE_BIRTH_SURPRISE_PROBE_H
