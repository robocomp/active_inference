/*
 * table_geometry.h — pure footprint-geometry + uncertainty helpers for table_concept (header-only).
 *
 ...
 * fitted to the same physical table. No state, no DSR — pure math on the belief covariance + the fitted state.
 */
...
#include "table_instance.h"   // rc::TableInstance (belief covariance)
#include "table_model.h"      // rc::TableState
...
inline float belief_uncertainty(const rc::TableInstance& inst)
{
...
// Two tables cannot share physical space. The footprint is the oriented rectangle (cx,cy,w,h,yaw) in the room
...
inline std::array<Eigen::Vector2f, 4> footprint_corners(const rc::TableState& s)
{
...
inline float footprint_overlap_ratio(const rc::TableState& a, const rc::TableState& b)
{
